use deadlib_render_backend_gl::bench_support;
...
fn main() {
    let filters = bench_support::filter_handles(EVENTS);
    print_pair(
        "OpenGL render-target filter updates",
        EVENTS,
        &measure(512, || bench_support::filters_old(black_box(&filters))),
        &measure(512, || bench_support::filters_new(black_box(&filters))),
    );

    let states = bench_support::state_events(EVENTS);
    print_pair(
        "OpenGL legacy offscreen state updates",
        EVENTS,
        &measure(512, || bench_support::states_old(black_box(&states))),
        &measure(512, || bench_support::states_new(black_box(&states))),
    );

    let vertices = bench_support::vertex_events(EVENTS);
    print_pair(
        "OpenGL draw vertex accounting",
        EVENTS,
        &measure(2_048, || bench_support::vertices_old(black_box(&vertices))),
        &measure(2_048, || bench_support::vertices_new(black_box(&vertices))),
    );
}
