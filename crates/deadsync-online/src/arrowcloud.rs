...
use deadsync_score::{
    ...
    arrowcloud_submit_stats_from_live_or_results, arrowcloud_submit_ui_status,
    arrowcloud_target_user_ids, arrowcloud_user_id, gameplay_run_failed,
    log_arrowcloud_bulk_import_event, run_arrowcloud_bulk_import_pack_groups,
...
where
    RuntimeProfile: GameplayProfileData,
{
...
pub fn submit_stats_from_runtime<RuntimeProfile, OverlayActor, CapturedActor, StateDelta>(
    gs: &deadsync_gameplay::GameplayRuntimeState<
        RuntimeProfile,
        OverlayActor,
        CapturedActor,
    ...
    RuntimeProfile: GameplayProfileData,
{
    ...
    arrowcloud_submit_stats_from_live_or_results(
        live_submit_stats_from_runtime(gs, player_idx),
        fail_time_ns,
        &gs.notes()[start..end],
...
pub fn payload_from_runtime<RuntimeProfile, OverlayActor, CapturedActor, StateDelta>(
    gs: &deadsync_gameplay::GameplayRuntimeState<
        RuntimeProfile,
        OverlayActor,
        CapturedActor,
...
pub fn submit_gameplay_from_runtime<RuntimeProfile, OverlayActor, CapturedActor, StateDelta, A, L>(
    gs: &deadsync_gameplay::GameplayRuntimeState<
        RuntimeProfile,
        OverlayActor,
        CapturedActor,
    ...
    L: Fn(&str) -> bool,
{
    ...
    submit_gameplay_players(input, players, cache_success, after_submit)
}

fn cache_submit_success_from_app_runtime(job: &ArrowCloudSubmitJob) {
    if let Some(profile_id) = job.profile_id.as_deref() {
        deadsync_profile::app_runtime::write_ac_submit_scores_for_id(
            profile_id,
            job.payload.hash.as_str(),
            job.itg_percent,
            job.ex_percent,
