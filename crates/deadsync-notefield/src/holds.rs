...
mod tests {
    ...
    #[test]
    fn hold_mesh_pool_handles_retained_and_saturated_frames() {
        const PAIRS: usize = 8;
        let mut pool = HoldMeshBufferPool::new(true, PAIRS, 12);
        ...
        for (frame_index, demand) in [1, 3, 5, 8, 2, 9, 0, 4, 8, 1].into_iter().enumerate() {
            let slot = frame_index % retained.len();
            retained[slot].clear();
            pool.begin_frame();
            ...
            for pair_index in 0..demand {
                ...
                let pair = pool.with_pair(|diffuse, glow| {
                ...
                match pair {
                    Some((start, lengths)) => {
                        acquired += 1;
                        assert_eq!(lengths, (vertex_count, vertex_count / 2));
                        let shared = pool.shared_pair(start);
                        assert_eq!(shared.0.len(), vertex_count);
                        assert_eq!(shared.1.len(), vertex_count / 2);
                        retained[slot].push(shared);
                    }
                    None => expected_saturations += 1,
                }
            }
            expected_high_water = expected_high_water.max(acquired);
        }

        assert_eq!(pool.stats.saturated_pairs, expected_saturations);
        assert_eq!(pool.stats.high_water_pairs, expected_high_water);
    }
