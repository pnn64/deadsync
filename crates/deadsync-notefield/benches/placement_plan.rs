use deadsync_notefield::{PlacementBench, PlacementBenchFrame};
...
// pointer/layout and only observes successful calls through independent atomics.
...
fn run(fixed_bytes: usize, mut frame_fn: impl FnMut(usize) -> PlacementBenchFrame) -> BenchResult {
    ...
    for frame in WARMUP_FRAMES..WARMUP_FRAMES + MEASURE_FRAMES {
        ...
        output.placements = output.placements.wrapping_add(current.placements);
    }
...
fn read_cycles() -> u64 {
    // SAFETY: the fence and timestamp instructions do not access memory; they
    // serialize this thread's measured instruction interval.
    unsafe {
