use deadsync_notefield::{CmodTimingBench, CmodTimingBenchFrame};
...
// independent counters only observe successful allocation and growth calls.
...
struct BenchResult {
    ...
    alloc: AllocSnapshot,
    frame_ns: Vec<u64>,
    output: CmodTimingBenchFrame,
}

fn main() {
    let bench = CmodTimingBench::default();
    let old = run(&bench, false);
    let new = run(&bench, true);
    assert_eq!(new.output, old.output, "old/new output checksum mismatch");

    println!("notefield CMod timing microbenchmark");
    println!("96 visible notes, 512 BPM segments plus stops/delays/warps, {MEASURE_FRAMES} frames");
    print_result("old: timing query/note", &old);
    print_result("new: song-load cache", &new);
    println!(
...
fn run(bench: &CmodTimingBench, cached: bool) -> BenchResult {
    for frame in 0..WARMUP_FRAMES {
        black_box(if cached {
            bench.new_frame(frame)
        } else {
            bench.old_frame(frame)
        });
    }
    ...
    for frame in WARMUP_FRAMES..WARMUP_FRAMES + MEASURE_FRAMES {
        ...
        let current = black_box(if cached {
            bench.new_frame(frame)
        } else {
            bench.old_frame(frame)
        });
...
fn print_result(name: &str, result: &BenchResult) {
    ...
    println!(
        "{:<24} p50 {:>8} ns p95 {:>8} ns p99 {:>8} ns worst {:>8} ns",
        "sampled frame cost",
