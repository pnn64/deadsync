use deadsync_import::xml::{XmlNode, bench_support};
...
fn measure(items_per_op: usize, mut operation: impl FnMut() -> u64) -> BenchResult {
    ...
    let allocation_runs = ITERATIONS / 10;
...
fn print_result(label: &str, result: &BenchResult) {
    ...
    println!(
        "  {label:<3} {:>11.2} ns/op  {:>11.2} cycles/op  {:>11.2} p95 ns  \
         {:>7.2} Mnode/s  {:>8.1} alloc/op  {:>6.1} realloc/op  \
...
fn fixture(kind: FixtureKind) -> String {
    let mut xml = String::with_capacity(NODES * 64);
    xml.push_str("<Stats>");
    for index in 0..NODES {
        match kind {
            FixtureKind::DirectText => {
                use std::fmt::Write as _;
                write!(xml, "<Score>{index:08}-plain-score-text</Score>").unwrap();
            }
            FixtureKind::ClosingNames => xml.push_str("<Empty></Empty>"),
            FixtureKind::TrimmedText => {
                use std::fmt::Write as _;
                write!(xml, "<Score>   {index:08}-padded-score-text   </Score>").unwrap();
            }
        }
    }
    xml.push_str("</Stats>");
...
fn parsed_checksum(
    parse: fn(&str) -> Result<XmlNode, deadsync_import::xml::XmlError>,
    xml: &str,
) -> u64 {
    let root = parse(black_box(xml)).expect("benchmark fixture parses");
    let first = root.children.first().map_or(0, |node| node.text.len());
    let last = root.children.last().map_or(0, |node| node.text.len());
    black_box(&root);
    (root.children.len() as u64)
        .wrapping_mul(1_000_003)
        .wrapping_add(root.tag.len() as u64)
        .wrapping_add((first as u64) << 16)
        .wrapping_add(last as u64)
}

fn assert_equivalent(xml: &str) {
    let old = bench_support::parse_old(xml).expect("legacy fixture parses");
    let new = bench_support::parse_new(xml).expect("optimized fixture parses");
    assert_eq!(new, old);
}

fn main() {
    let direct = fixture(FixtureKind::DirectText);
    let closing = fixture(FixtureKind::ClosingNames);
    let trimmed = fixture(FixtureKind::TrimmedText);
    assert_equivalent(&direct);
    assert_equivalent(&closing);
    assert_equivalent(&trimmed);

    print_pair(
        "direct text decoding into the retained buffer",
        &measure(NODES, || parsed_checksum(bench_support::parse_old, &direct)),
        &measure(NODES, || {
            parsed_checksum(bench_support::parse_direct_text, &direct)
        }),
    );
    print_pair(
        "allocation-free closing-name scan",
        &measure(NODES, || {
            parsed_checksum(bench_support::parse_old, &closing)
        }),
        &measure(NODES, || {
            parsed_checksum(bench_support::parse_skipped_closing_names, &closing)
        }),
    );
    print_pair(
        "in-place retained text trimming",
        &measure(NODES, || {
            parsed_checksum(bench_support::parse_old, &trimmed)
        }),
        &measure(NODES, || {
            parsed_checksum(bench_support::parse_trimmed_in_place, &trimmed)
        }),
    );
}
