use deadsync_import::resolver::{normalize_song_dir, normalize_song_dir_legacy_for_bench};
...
const PATHS: [&str; 8] = [
    ...
    "AdditionalSongs/Technical Pack/Final Song",
    "Pack Without Root/Song Folder/",
    "Songs\\Windows Pack\\Windows Song\\",
...
fn main() {
    for path in PATHS {
        assert_eq!(
            normalize_song_dir_legacy_for_bench(path),
            normalize_song_dir(path)
        );
    }

    let old = measure(normalize_song_dir_legacy_for_bench);
    let new = measure(normalize_song_dir);
    assert_eq!(old.checksum, new.checksum);

    println!(
        "import song-directory normalization ({} paths x {RUNS} runs)",
        PATHS.len()
    );
...
fn measure(normalize: fn(&str) -> Option<(String, String)>) -> BenchResult {
    for _ in 0..500 {
        black_box(batch_checksum(normalize));
    }
    ...
    for run in 0..RUNS {
        checksum = checksum.rotate_left(7) ^ black_box(batch_checksum(normalize)) ^ run as u64;
    }
    BenchResult {
...
fn batch_checksum(normalize: fn(&str) -> Option<(String, String)>) -> u64 {
    PATHS.iter().fold(0_u64, |checksum, path| {
        let value = normalize(black_box(path));
        let length = value
            .as_ref()
            .map_or(0, |(pack, song)| pack.len() + song.len());
        checksum.rotate_left(3) ^ length as u64
    })
}
