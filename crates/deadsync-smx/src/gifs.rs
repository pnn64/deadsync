...
pub fn select_variant(
    ...
    if let Some(bpm) = song_bpm.filter(|b| b.is_finite() && *b > 0.0) {
        // Track both candidates in one pass. The previous iterator chain had
        // to scan every tagged variant a second time whenever the song was
        // faster than the highest reference tempo.
        let mut at_or_above = None;
        let mut highest = None;
        for variant in variants {
            let Some(reference) = variant.ref_bpm else {
                continue;
            };
            if reference >= bpm {
                if at_or_above.is_none_or(|(best, _): (f32, &BackgroundVariant)| {
                    reference.total_cmp(&best).is_lt()
                }) {
                    at_or_above = Some((reference, variant));
                }
            } else if highest.is_none_or(|(best, _): (f32, &BackgroundVariant)| {
                !reference.total_cmp(&best).is_lt()
            }) {
                highest = Some((reference, variant));
            }
        }
        let pick = at_or_above.or(highest);
...
pub fn tint_full_pad(anim: &FullPadAnim, target_rgb: [u8; 3]) -> FullPadAnim {
    let tint_frame = |frame: &PanelFrame| -> PanelFrame {
        ...
        for px in out.as_chunks_mut::<3>().0 {
            px[0] = tint_channel(px[0], target_rgb[0]);
            px[1] = tint_channel(px[1], target_rgb[1]);
            px[2] = tint_channel(px[2], target_rgb[2]);
        }
        out
    };
    FullPadAnim {
...
/// Exact integer equivalent of `round(source * target / 255)`. Since 255 is
/// odd, the rational result can never land exactly on a half-integer.
#[inline]
const fn tint_channel(source: u8, target: u8) -> u8 {
    ((source as u16 * target as u16 + 127) / 255) as u8
}
...
pub fn saturate_for_leds(rgb: [u8; 3]) -> [u8; 3] {
    let max_byte = rgb.into_iter().max().unwrap_or(0);
    if max_byte == 0 {
        return rgb;
    }
    let max = f32::from(max_byte);
    rgb.map(|c| {
        // Every color has at least one peak channel, and zeros are also
        // common in LED palettes. Both are exact fixed points of the curve.
        if c == 0 || c == max_byte {
            c
        } else {
            (max * (f32::from(c) / max).powf(LED_SATURATION_GAMMA)).round() as u8
        }
    })
}
...
pub mod bench_support {
    use super::{
        BackgroundVariant, FullPadAnim, LED_SATURATION_GAMMA, PANEL_RGB_BYTES, PanelFrame,
        saturate_for_leds, select_variant, tint_full_pad,
    };
    ...
    const TINT_FRAMES: usize = 128;
    const SATURATION_PALETTE: [[u8; 3]; 16] = [
        [0, 0, 0],
        [32, 32, 32],
        [128, 128, 128],
        [255, 255, 255],
    ...
    fn variants() -> &'static [BackgroundVariant] {
        static VARIANTS: OnceLock<Vec<BackgroundVariant>> = OnceLock::new();
        VARIANTS.get_or_init(|| {
            (0..VARIANT_COUNT)
                .map(|index| BackgroundVariant {
                    anim: Arc::new(FullPadAnim {
                        panels: std::array::from_fn(|_| vec![[index as u8; PANEL_RGB_BYTES]]),
                        durations: vec![1.0 / 30.0],
                        loop_frame: 0,
                        beats_per_loop: None,
                    }),
                    ref_bpm: Some(60.0 + index as f32 * 8.0),
                })
                .collect()
        })
    }

    fn select_variant_old(
        variants: &[BackgroundVariant],
        song_bpm: Option<f32>,
    ) -> Option<Arc<FullPadAnim>> {
        if let Some(bpm) = song_bpm.filter(|b| b.is_finite() && *b > 0.0) {
            let at_or_above = variants
                .iter()
                .filter_map(|variant| variant.ref_bpm.map(|y| (y, variant)))
                .filter(|(reference, _)| *reference >= bpm)
            ...
            if let Some((_, variant)) = pick {
                return Some(variant.anim.clone());
            }
        }
        variants
            .iter()
            .min_by(|a, b| super::ref_bpm_sort_key(a).total_cmp(&super::ref_bpm_sort_key(b)))
            .map(|variant| variant.anim.clone())
    }

    fn variant_selection(probes: usize, old: bool) -> u64 {
        let variants = black_box(variants());
        let mut checksum = 0_u64;
        for index in 0..probes {
            // Three quarters of selections exercise the above-range fallback
            // that formerly scanned every merged variant twice.
            let bpm = if index & 3 == 0 {
                61.0 + (index % (VARIANT_COUNT * 8 - 1)) as f32
            } else {
                300.0 + (index & 31) as f32
            };
            let selected = if old {
                select_variant_old(variants, Some(black_box(bpm)))
            } else {
                select_variant(variants, Some(black_box(bpm)))
            }
            .expect("benchmark variants are non-empty");
            checksum = checksum.rotate_left(5) ^ Arc::as_ptr(&selected) as usize as u64;
        }
        checksum
    }

    fn tint_source() -> &'static FullPadAnim {
        static ANIM: OnceLock<FullPadAnim> = OnceLock::new();
        ANIM.get_or_init(|| FullPadAnim {
            panels: std::array::from_fn(|panel| {
                (0..TINT_FRAMES)
                    .map(|frame| {
                        std::array::from_fn(|byte| {
                            panel
                                .wrapping_mul(97)
                                .wrapping_add(frame.wrapping_mul(53))
                                .wrapping_add(byte.wrapping_mul(29))
                                as u8
                        })
                    })
                    .collect()
            }),
            durations: vec![1.0 / 30.0; TINT_FRAMES],
            loop_frame: 17,
            beats_per_loop: Some(8.0),
        })
    }

    fn tint_full_pad_old(anim: &FullPadAnim, target_rgb: [u8; 3]) -> FullPadAnim {
        let scale = target_rgb.map(|channel| f32::from(channel) / 255.0);
        let tint_frame = |frame: &PanelFrame| -> PanelFrame {
            let mut out = *frame;
        ...
        FullPadAnim {
            panels: std::array::from_fn(|panel| {
                anim.panels[panel].iter().map(tint_frame).collect()
            }),
            durations: anim.durations.clone(),
            loop_frame: anim.loop_frame,
            beats_per_loop: anim.beats_per_loop,
    ...
    fn tint_checksum(anim: &FullPadAnim) -> u64 {
        let mut checksum = anim.loop_frame as u64;
        for panel in 0..PANELS {
            for frame in [0, TINT_FRAMES / 2, TINT_FRAMES - 1] {
                for &byte in &anim.panels[panel][frame] {
                    checksum = checksum.rotate_left(3) ^ u64::from(byte);
                }
            }
        }
        checksum ^ anim.durations.len() as u64
    }

    fn tint(old: bool) -> u64 {
        let source = black_box(tint_source());
        let target = black_box([207, 87, 126]);
        let tinted = if old {
            tint_full_pad_old(source, target)
        } else {
            tint_full_pad(source, target)
        };
        black_box(&tinted);
        tint_checksum(&tinted)
    }

    fn saturate_old(rgb: [u8; 3]) -> [u8; 3] {
        let max = f32::from(rgb.into_iter().max().unwrap_or(0));
        if max <= 0.0 {
            return rgb;
        }
        rgb.map(|channel| {
            (max * (f32::from(channel) / max).powf(LED_SATURATION_GAMMA)).round() as u8
        })
    }

    fn saturation(colors: usize, old: bool) -> u64 {
        let mut checksum = 0_u64;
        for index in 0..colors {
            let rgb = black_box(SATURATION_PALETTE[index % SATURATION_PALETTE.len()]);
            let out = if old {
                saturate_old(rgb)
            } else {
                saturate_for_leds(rgb)
            };
    ...
    #[must_use]
    pub fn variant_selection_old(probes: usize) -> u64 {
        variant_selection(probes, true)
    }

    #[must_use]
    pub fn variant_selection_new(probes: usize) -> u64 {
        variant_selection(probes, false)
    }
    ...
    #[must_use]
    pub fn saturation_old(colors: usize) -> u64 {
        saturation(colors, true)
    }

    #[must_use]
    pub fn saturation_new(colors: usize) -> u64 {
        saturation(colors, false)
    }
}
...
mod tests {
    ...
    #[test]
    fn integer_tint_matches_float_reference_for_every_channel_pair() {
        for source in 0..=u8::MAX {
            for target in 0..=u8::MAX {
                let expected = (f32::from(source) * (f32::from(target) / 255.0)).round() as u8;
                assert_eq!(
                    tint_channel(source, target),
                    expected,
    ...
    #[test]
    fn saturation_fast_paths_match_powf_reference_for_every_channel_ratio() {
        let reference = |rgb: [u8; 3]| {
            let max = f32::from(rgb.into_iter().max().unwrap_or(0));
            if max <= 0.0 {
                rgb
            } else {
        ...
        // The transform is channel-independent once the maximum is known.
        // Exercise every possible (channel, maximum) pair in every position,
        // including the zero and peak fixed points taken by the fast path.
        for max in 0..=u8::MAX {
            for channel in 0..=max {
                for rgb in [
                    [max, channel, 0],
                    [0, max, channel],
                    [channel, 0, max],
                    ...
                    assert_eq!(saturate_for_leds(rgb), reference(rgb), "rgb={rgb:?}");
                }
            }
        }
    }
    ...
    #[test]
    fn one_pass_variant_selection_matches_two_scan_reference() {
        fn reference(
            variants: &[BackgroundVariant],
            song_bpm: Option<f32>,
        ) -> Option<Arc<FullPadAnim>> {
        ...
        let tempos = [None, Some(225.0), Some(90.0), Some(150.0), Some(225.0)];
        let variants: Vec<_> = tempos
            .into_iter()
            .map(|ref_bpm| BackgroundVariant {
                anim: dummy_full_pad(),
                ref_bpm,
            })
            .collect();
        for song_bpm in [
            ...
            let expected = reference(&variants, song_bpm);
            let actual = select_variant(&variants, song_bpm);
            assert_eq!(
                actual.is_some(),
                expected.is_some(),
                "song_bpm={song_bpm:?}"
            ...
            assert!(
                actual
                    .zip(expected)
                    .is_none_or(|(actual, expected)| Arc::ptr_eq(&actual, &expected)),
                "song_bpm={song_bpm:?}"
            );
        }
    }
