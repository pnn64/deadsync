use deadsync_smx::gifs::bench_support;
use std::alloc::{GlobalAlloc, Layout, System};
use std::hint::black_box;
use std::sync::atomic::{AtomicBool, AtomicU64, Ordering};
use std::time::Instant;

const VARIANT_PROBES: usize = 8_192;
const TINT_BYTES: usize = 9 * 128 * 75;
const SATURATION_COLORS: usize = 8_192;
...
fn measure(iterations: usize, mut operation: impl FnMut() -> u64) -> Row {
    for _ in 0..3 {
        black_box(operation());
    }
...
fn print_pair(title: &str, units: usize, old: &Row, new: &Row) {
    ...
    assert!(
        ...
        "{title} allocation behavior regressed"
    );
    assert!(
...
fn main() {
    print_pair(
        "SMX GIF variant selection",
        VARIANT_PROBES,
        &measure(32, || {
            bench_support::variant_selection_old(black_box(VARIANT_PROBES))
        }),
        &measure(32, || {
            bench_support::variant_selection_new(black_box(VARIANT_PROBES))
        }),
    );

    print_pair(
        "SMX full-pad GIF tint",
        TINT_BYTES,
        &measure(16, bench_support::tint_old),
        &measure(16, bench_support::tint_new),
    );

    print_pair(
        "SMX LED color saturation",
        SATURATION_COLORS,
        &measure(16, || {
            bench_support::saturation_old(black_box(SATURATION_COLORS))
        }),
        &measure(16, || {
            bench_support::saturation_new(black_box(SATURATION_COLORS))
        }),
    );
}
