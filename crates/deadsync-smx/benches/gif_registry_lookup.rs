use deadsync_smx::gifs::GifRegistryLookupBench;
...
fn main() {
    let registry = GifRegistryLookupBench::new();
    assert_eq!(registry.legacy_checksum(), registry.current_checksum());

    let old = measure(&registry, GifRegistryLookupBench::legacy_checksum);
    let new = measure(&registry, GifRegistryLookupBench::current_checksum);
    assert_eq!(old.checksum, new.checksum);

    println!("SMX GIF registry resolution ({LOOKUPS_PER_RUN} fallback cases x {RUNS} runs)");
...
fn measure(
    registry: &GifRegistryLookupBench,
    lookup: fn(&GifRegistryLookupBench) -> u64,
) -> BenchResult {
    for _ in 0..2_000 {
        black_box(lookup(black_box(registry)));
    }
    ...
    for run in 0..RUNS {
        checksum = checksum.rotate_left(7) ^ black_box(lookup(black_box(registry))) ^ run as u64;
    }
    BenchResult {
