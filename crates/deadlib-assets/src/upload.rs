...
impl UploadQueueBench for TextureUploadQueue {
    ...
    fn bench_pop(
        ...
        self.pop_next(budget, drained_uploads, drained_bytes)
            .map(|(key, upload)| (key.into_string(), upload.bytes))
    }
}
...
/// Models one already-decoded gameplay video frame entering the upload queue.
#[cfg(any(test, feature = "bench-support"))]
#[doc(hidden)]
pub struct VideoUploadKeyBenchmark {
    legacy_queue: TextureUploadQueue,
    shared_queue: TextureUploadQueue,
    owned_key: String,
    shared_key: Arc<str>,
    image: Arc<RgbaImage>,
}
...
#[cfg(any(test, feature = "bench-support"))]
impl Default for VideoUploadKeyBenchmark {
    fn default() -> Self {
        let owned_key = "Songs/Benchmark Pack/Video Song/background.mp4".to_owned();
        Self {
            legacy_queue: TextureUploadQueue::default(),
            shared_queue: TextureUploadQueue::default(),
            shared_key: Arc::from(owned_key.as_str()),
            owned_key,
...
#[cfg(any(test, feature = "bench-support"))]
impl VideoUploadKeyBenchmark {
    const BUDGET: TextureUploadBudget = TextureUploadBudget {
        max_uploads: 1,
        max_bytes: usize::MAX,
    };

    pub fn legacy_frame(&mut self) -> usize {
        self.legacy_queue.push(
            self.owned_key.clone(),
            Arc::clone(&self.image),
            SamplerDesc::default(),
        );
        let (key, upload) = self
            .legacy_queue
            .pop_next(Self::BUDGET, 0, 0)
            .expect("benchmark upload was queued");
        upload_key_checksum(key.as_str()) as usize ^ upload.bytes
    }

    pub fn shared_frame(&mut self) -> usize {
        self.shared_queue.push_shared(
            Arc::clone(&self.shared_key),
            Arc::clone(&self.image),
            SamplerDesc::default(),
        );
        let (key, upload) = self
...
mod tests {
    ...
    #[test]
    fn shared_video_upload_key_preserves_queue_identity_and_behavior() {
        let mut legacy = VideoUploadKeyBenchmark::default();
        let mut shared = VideoUploadKeyBenchmark::default();
        for _ in 0..4 {
            assert_eq!(legacy.legacy_frame(), shared.shared_frame());
        }
