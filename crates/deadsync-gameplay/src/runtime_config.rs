...
pub fn music_time_ns_from_song_clock(
    ...
    if snapshot.timing_diag_enabled {
        let use_host_clock = snapshot.valid_at_host_nanos != 0 && captured_host_nanos != 0;
        log::debug!(
            "AUDIO_DIAG snap_age_ms={:.3} path={} callback_gap_ms={:.3} snapshot_song_time={:.6} slope={:.6} snapshot_host_nanos={} captured_host_nanos={}",
            if use_host_clock {
                let dt_nanos =
                    i128::from(captured_host_nanos) - i128::from(snapshot.valid_at_host_nanos);
                dt_nanos as f64 * 1e-6
            } else if let Some(age) = snapshot.valid_at.checked_duration_since(captured_at) {
            ...
            if use_host_clock { "host" } else { "instant" },
            snapshot.timing_diag_callback_gap_ns as f64 * 1e-6,
            song_time_ns_to_seconds(snapshot.song_time_ns),
            normalized_song_rate(snapshot.seconds_per_second),
