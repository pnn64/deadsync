...
#[derive(Clone, Copy, Debug)]
struct DisplayClockFrameTiming {
    slope: f32,
    delta_ns: SongTimeNs,
}
...
pub struct FrameStableDisplayClock {
    ...
    rate_limits_slope_bits: u32,
    rate_limits: DisplayClockRateLimits,
    frame_delta_time_bits: u32,
    frame_rate_input_bits: u32,
    frame_timing: DisplayClockFrameTiming,
}
...
impl FrameStableDisplayClock {
    ...
    pub const fn new(time_ns: SongTimeNs) -> Self {
        Self {
            ...
            rate_limits_slope_bits: u32::MAX,
            rate_limits: DisplayClockRateLimits {
                max_error_ns: 0,
                max_step_ns: 0,
                max_lag_ns: 0,
            ...
            frame_delta_time_bits: u32::MAX,
            frame_rate_input_bits: u32::MAX,
            frame_timing: DisplayClockFrameTiming {
                slope: 1.0,
                delta_ns: 0,
            },
    ...
    #[inline(always)]
    fn rate_limits(&mut self, slope: f32) -> DisplayClockRateLimits {
        let bits = slope.to_bits();
        if self.rate_limits_slope_bits != bits {
            self.rate_limits_slope_bits = bits;
            let max_step_ns = i128::from(scaled_song_time_ns(DISPLAY_CLOCK_MAX_STEP_S, slope));
            self.rate_limits = DisplayClockRateLimits {
    ...
    ) -> DisplayClockFrameTiming {
        let delta_bits = delta_time.to_bits();
        let rate_bits = seconds_per_second.to_bits();
        if self.frame_delta_time_bits != delta_bits || self.frame_rate_input_bits != rate_bits {
            self.frame_delta_time_bits = delta_bits;
            self.frame_rate_input_bits = rate_bits;
            let slope = normalized_song_rate(seconds_per_second);
            self.frame_timing = DisplayClockFrameTiming {
                slope,
                delta_ns: scaled_song_time_ns(delta_time, slope),
            };
        }
        self.frame_timing
    }
...
pub fn frame_stable_display_clock_step(
    ...
    let timing = display_clock.frame_timing(delta_time, seconds_per_second);
    let slope = timing.slope;
    ...
    let limits = display_clock.rate_limits(slope);
    ...
    let advanced_ns = i128::from(previous_display_time_ns) + i128::from(timing.delta_ns);
    let correction_alpha = display_clock.correction_alpha(delta_time);
    let mut corrected_ns = advanced_ns
        + ((i128::from(target_display_time_ns) - advanced_ns) as f64
            * f64::from(correction_alpha))
        .round() as i128;
    if target_delta_ns.abs() > limits.target_jump_ns {
        note_event(display_clock_step_event(
...
/// Pre-optimization display-clock math retained for parity tests and release benchmarks.
#[cfg(any(test, feature = "bench-support"))]
#[doc(hidden)]
pub fn reference_frame_stable_display_clock_step(
    display_clock: &mut FrameStableDisplayClock,
    target_display_time_ns: SongTimeNs,
    delta_time: f32,
    seconds_per_second: f32,
    ...
    let previous_error_over_threshold = display_clock.error_over_threshold;
    let target_delta_ns = i128::from(target_display_time_ns) - i128::from(previous_display_time_ns);
    let limits = DisplayClockRateLimits {
        max_error_ns: i128::from(scaled_song_time_ns(DISPLAY_CLOCK_RESET_ERROR_S, slope)),
    ...
    let mut corrected_ns = advanced_ns
        ...
        .round() as i128;
    let max_step_ns = i128::from(scaled_song_time_ns(DISPLAY_CLOCK_MAX_STEP_S, slope));
    let target_jump_ns = (max_step_ns as f64 * 2.0).round() as i128;
    let clamp_step_ns = (max_step_ns as f64 * 1.2).round() as i128;
    if target_delta_ns.abs() > target_jump_ns {
        note_event(display_clock_step_event(
            DisplayClockDiagEventKind::TargetJump,
            target_display_time_ns,
            previous_display_time_ns,
...
pub mod display_clock_bench_support {
    ...
    fn rate_limits_checksum(limits: DisplayClockRateLimits) -> u64 {
        (limits.max_error_ns as u64)
            ^ (limits.max_step_ns as u64).rotate_left(11)
            ^ (limits.max_lag_ns as u64).rotate_left(23)
            ^ (limits.max_lead_ns as u64).rotate_left(37)
            ^ (limits.target_jump_ns as u64).rotate_left(47)
            ^ (limits.clamp_step_ns as u64).rotate_left(59)
    }

    fn frame_timing_checksum(timing: DisplayClockFrameTiming) -> u64 {
        u64::from(timing.slope.to_bits()) ^ (timing.delta_ns as u64).rotate_left(31)
    }

    #[must_use]
    pub fn rate_limits_old(evaluations: usize, slope: f32) -> u64 {
        let mut scaled_bits = u32::MAX;
        let mut threshold_bits = u32::MAX;
        let mut limits = DisplayClockRateLimits {
            max_error_ns: 0,
            max_step_ns: 0,
            max_lag_ns: 0,
            max_lead_ns: 0,
        ...
        for _ in 0..evaluations {
            ...
            if scaled_bits != bits {
                scaled_bits = bits;
                limits.max_error_ns = i128::from(scaled_song_time_ns(
                    DISPLAY_CLOCK_RESET_ERROR_S,
                    slope,
                ));
                limits.max_step_ns =
                    i128::from(scaled_song_time_ns(DISPLAY_CLOCK_MAX_STEP_S, slope));
                limits.max_lag_ns =
                    i128::from(scaled_song_time_ns(DISPLAY_CLOCK_MAX_LAG_S, slope));
                limits.max_lead_ns =
                    i128::from(scaled_song_time_ns(DISPLAY_CLOCK_MAX_LEAD_S, slope));
            }
            if threshold_bits != bits {
                threshold_bits = bits;
                let max_step_ns =
                    i128::from(scaled_song_time_ns(DISPLAY_CLOCK_MAX_STEP_S, slope));
                limits.target_jump_ns = (max_step_ns as f64 * 2.0).round() as i128;
                limits.clamp_step_ns = (max_step_ns as f64 * 1.2).round() as i128;
            }
            checksum = checksum.wrapping_add(rate_limits_checksum(limits));
        }
        checksum
    }

    #[must_use]
    pub fn rate_limits_new(evaluations: usize, slope: f32) -> u64 {
        let mut clock = FrameStableDisplayClock::new(0);
        let mut checksum = 0_u64;
        for _ in 0..evaluations {
            let limits = clock.rate_limits(black_box(slope));
            checksum = checksum.wrapping_add(rate_limits_checksum(limits));
        }
        checksum
    }
    ...
    #[must_use]
    pub fn frame_delta_old(evaluations: usize, delta_time: f32, slope: f32) -> u64 {
        let mut checksum = 0_u64;
        for _ in 0..evaluations {
            let delta_ns = scaled_song_time_ns(black_box(delta_time), black_box(slope));
            checksum = checksum.wrapping_add(delta_ns as u64);
        }
        checksum
    }

    #[must_use]
    pub fn frame_delta_new(evaluations: usize, delta_time: f32, slope: f32) -> u64 {
        let mut clock = FrameStableDisplayClock::new(0);
        let mut checksum = 0_u64;
        for _ in 0..evaluations {
            let delta_ns = clock
                .frame_timing(black_box(delta_time), black_box(slope))
                .delta_ns;
            checksum = checksum.wrapping_add(delta_ns as u64);
        }
        checksum
    ...
    #[must_use]
    pub fn frame_timing_old(
        evaluations: usize,
        delta_time: f32,
        seconds_per_second: f32,
    ) -> u64 {
        let mut delta_time_bits = u32::MAX;
        let mut delta_slope_bits = u32::MAX;
        let mut delta_ns = 0;
        let mut checksum = 0_u64;
        for _ in 0..evaluations {
            let delta_time = black_box(delta_time);
            let slope = normalized_song_rate(black_box(seconds_per_second));
            let slope_bits = slope.to_bits();
            let current_delta_bits = delta_time.to_bits();
            if delta_time_bits != current_delta_bits || delta_slope_bits != slope_bits {
                delta_time_bits = current_delta_bits;
                delta_slope_bits = slope_bits;
                delta_ns = scaled_song_time_ns(delta_time, slope);
            }
            checksum = checksum.wrapping_add(frame_timing_checksum(DisplayClockFrameTiming {
                slope,
                delta_ns,
            }));
        }
    ...
    pub fn frame_timing_new(
        ...
        for _ in 0..evaluations {
            let timing = clock.frame_timing(
                ...
                black_box(seconds_per_second),
            );
            checksum = checksum.wrapping_add(frame_timing_checksum(timing));
        }
        checksum
    }
}
