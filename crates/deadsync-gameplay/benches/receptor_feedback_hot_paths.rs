use deadsync_gameplay::{ReceptorFeedbackBenchmark, ReferenceReceptorFeedbackBenchmark};
...
fn schedule(frame: usize, pattern: Pattern, mut pulse: impl FnMut(usize, f32, f32, f32)) {
    match pattern {
        Pattern::Idle => {}
        Pattern::Sparse if frame.is_multiple_of(30) => {
            pulse((frame / 30 * 7) % LANES, 0.08, 0.20, 0.10);
        }
        Pattern::Chord if frame.is_multiple_of(60) => {
            for col in [0, 2, 5, 7] {
                pulse(col, 0.12, 0.24, 0.14);
            }
        }
        Pattern::Dense if frame == 0 => {
            for col in 0..LANES {
                pulse(col, f32::INFINITY, f32::INFINITY, f32::INFINITY);
            }
        }
        Pattern::Sparse | Pattern::Chord | Pattern::Dense => {}
    }
}
...
fn run(title: &str, iterations: usize, pattern: Pattern) {
    let mut old = ReferenceReceptorFeedbackBenchmark::new(LANES, 2, LANES / 2);
    let old_bytes = ReferenceReceptorFeedbackBenchmark::state_bytes();
    let mut old_frame = 0usize;
    let old_result = measure(iterations, || {
        schedule(old_frame, pattern, |col, press, lift, bop| {
            old.pulse(col, press, lift, bop);
        });
        old.tick(1.0 / 120.0);
        old_frame = old_frame.wrapping_add(1);
        old.probe()
    });
    let old_state_checksum = old.checksum();

    let mut new = ReceptorFeedbackBenchmark::new(LANES, 2, LANES / 2);
    let new_bytes = ReceptorFeedbackBenchmark::state_bytes();
    let mut new_frame = 0usize;
    let new_result = measure(iterations, || {
        schedule(new_frame, pattern, |col, press, lift, bop| {
            new.pulse(col, press, lift, bop);
        });
        new.tick(1.0 / 120.0);
        new_frame = new_frame.wrapping_add(1);
        new.probe()
    });
    let new_state_checksum = new.checksum();

    assert_eq!(old_result.checksum, new_result.checksum, "{title} diverged");
    assert_eq!(
        old_state_checksum, new_state_checksum,
    ...
    println!("\n{title}");
    println!(
        "  state storage: old={old_bytes} B, new={new_bytes} B ({:+.2}%)",
...
fn main() {
    run("idle ten-lane feedback", 3_000_000, Pattern::Idle);
    run("sparse rotating taps", 2_000_000, Pattern::Sparse);
    run("four-lane chords", 1_500_000, Pattern::Chord);
    run("dense ten-lane animations", 1_000_000, Pattern::Dense);
}
