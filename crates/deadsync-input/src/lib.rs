...
#[inline(always)]
#[must_use]
fn action_from_ini_key_lower(key: &str) -> Option<VirtualAction> {
    use VirtualAction::{
...
mod tests {
    ...
    fn input_debounce_parses_config_units() {
        ...
        assert_eq!(parse_input_debounce_seconds("-25Ms"), Some(0.0));
    ...
    #[test]
    fn gamepad_code_bindings_format_canonical_tokens() {
        let cases = [
            (
                GamepadCodeBinding {
                    code_u32: 0,
            ...
            (
                ...
                "PadCode[0xDEADBEEF]@123#ABABABABABABABABABABABABABABABAB",
            ),
            (
                GamepadCodeBinding {
                    code_u32: 42,
                ...
                "PadCode[0x0000002A]#000102030405060708090A0B0C0D0E0F",
            ),
        ];
        for (binding, expected) in cases {
            assert_eq!(gamepad_code_binding_to_token(binding), expected);
        }
    }
    ...
    fn virtual_actions_round_trip_through_ini_names() {
        for action in ALL_VIRTUAL_ACTIONS {
            ...
            assert_eq!(action_from_ini_key(key), Some(action));
            assert_eq!(action_from_ini_key(&key.to_ascii_uppercase()), Some(action));
        }
        assert_eq!(action_from_ini_key("p1_coin"), Some(VirtualAction::p1_coin));
        for invalid in ["", "P1_Menu_Up", "é", "System_FastForward_extra"] {
            assert_eq!(action_from_ini_key(invalid), None, "input: {invalid:?}");
        }
    }
