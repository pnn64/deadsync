...
const PATHS: [&str; 10] = [
    ...
    "Songs/Pack/Song/movie.WeBm",
    "Songs/Pack/Song/music.FLAC",
    "Songs/Pack/Song/preview.OGG",
...
fn main() {
    for path in PATHS.map(Path::new) {
        assert_eq!(
            ...
            song_lua_path_classification_for_bench(path)
        );
    }
    let old = measure(song_lua_path_classification_legacy_for_bench);
    let new = measure(song_lua_path_classification_for_bench);
    assert_eq!(old.checksum, new.checksum);

    println!(
        "SongLua media path classification ({} paths x {RUNS} runs)",
        PATHS.len()
    );
