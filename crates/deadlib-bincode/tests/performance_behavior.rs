...
#[test]
fn reusable_hash_map_reuses_byte_buffer_key_allocations() {
    let values = (0..128u64)
        .map(|index| {
            let mut key = index.to_le_bytes().to_vec();
            key.extend((0..88u64).map(|offset| index.wrapping_mul(31).wrapping_add(offset) as u8));
            (key, index.wrapping_mul(1_000_003))
        })
        .collect::<HashMap<_, _>>();
    let config = bincode::config::standard();
    let encoded = bincode::encode_to_vec(&values, config).unwrap();
...
#[test]
fn reusable_hash_map_reuses_byte_buffer_value_allocations() {
    let values = (0..128u64)
        .map(|index| {
            let mut value = index.to_le_bytes().to_vec();
            value
                .extend((0..88u64).map(|offset| index.wrapping_mul(31).wrapping_add(offset) as u8));
            (index.wrapping_mul(1_000_003), value)
        })
        .collect::<HashMap<_, _>>();
    let config = bincode::config::standard();
    let encoded = bincode::encode_to_vec(&values, config).unwrap();
...
#[test]
fn reusable_hash_map_reuses_mixed_string_and_byte_buffers() {
    let values = (0..64u64)
        .map(|index| {
            let mut value = index.to_le_bytes().to_vec();
            value.extend((0..88u64).map(|offset| index.wrapping_add(offset) as u8));
            (format!("chart-{index:03}-dead-sync"), value)
        })
        .collect::<HashMap<_, _>>();
    let config = bincode::config::standard();
    let encoded = bincode::encode_to_vec(&values, config).unwrap();
...
fn string_vector(index: usize) -> Vec<String> {
    (0..4)
        .map(|part| format!("chart-{index:03}-part-{part}-{}", "dead-sync".repeat(4)))
        .collect()
}

fn warmed_string_vector(index: usize) -> Vec<String> {
    let mut values = Vec::with_capacity(8);
    for part in 0..4 {
        let mut value = String::with_capacity(128);
        value.push_str(&format!("discarded-{index}-{part}"));
        values.push(value);
    }
    values
}

fn string_vector_allocations<'a>(
    values: impl IntoIterator<Item = &'a Vec<String>>,
) -> (HashSet<usize>, HashSet<usize>) {
    let mut outer = HashSet::new();
    let mut strings = HashSet::new();
    for value in values {
        outer.insert(value.as_ptr() as usize);
        strings.extend(value.iter().map(|string| string.as_ptr() as usize));
    }
    (outer, strings)
}

#[test]
fn reusable_hash_map_reuses_string_vector_key_allocations() {
    let values = (0..64)
        .map(|index| (string_vector(index), index as u64))
        .collect::<HashMap<_, _>>();
    let config = bincode::config::standard();
    let encoded = bincode::encode_to_vec(&values, config).unwrap();
    let mut decoded = (0..values.len())
        .map(|index| (warmed_string_vector(index), u64::MAX))
        .collect::<HashMap<_, _>>();
    let capacity = decoded.capacity();
    let allocations = string_vector_allocations(decoded.keys());

    let used = bincode::decode_from_slice_into_hash_map(&encoded, &mut decoded, config).unwrap();
    assert_eq!((&decoded, used), (&values, encoded.len()));
    assert_eq!(decoded.capacity(), capacity);
    assert_eq!(string_vector_allocations(decoded.keys()), allocations);
}

#[test]
fn reusable_hash_map_reuses_string_vector_value_allocations() {
    let values = (0..64)
        .map(|index| (index as u64, string_vector(index)))
        .collect::<HashMap<_, _>>();
    let config = bincode::config::standard();
    let encoded = bincode::encode_to_vec(&values, config).unwrap();
    let mut decoded = (0..values.len())
        .map(|index| (index as u64, warmed_string_vector(index)))
        .collect::<HashMap<_, _>>();
    let capacity = decoded.capacity();
    let allocations = string_vector_allocations(decoded.values());

    let used = bincode::decode_from_slice_into_hash_map(&encoded, &mut decoded, config).unwrap();
    assert_eq!((&decoded, used), (&values, encoded.len()));
    assert_eq!(decoded.capacity(), capacity);
    assert_eq!(string_vector_allocations(decoded.values()), allocations);
}

#[test]
fn reusable_hash_map_reuses_mixed_string_and_string_vector_allocations() {
    let values = (0..64)
        .map(|index| (format!("key-{index:03}-dead-sync"), string_vector(index)))
        .collect::<HashMap<_, _>>();
    let config = bincode::config::standard();
    let encoded = bincode::encode_to_vec(&values, config).unwrap();
    let mut decoded = (0..values.len())
        .map(|index| {
            let mut key = String::with_capacity(128);
            key.push_str(&format!("discarded-key-{index}"));
            (key, warmed_string_vector(index))
        })
        .collect::<HashMap<_, _>>();
    let key_allocations = decoded
        .keys()
        .map(|key| key.as_ptr() as usize)
        .collect::<HashSet<_>>();
    let value_allocations = string_vector_allocations(decoded.values());
    ...
    assert_eq!(
        string_vector_allocations(decoded.values()),
        value_allocations
    );
}

#[test]
fn reusable_string_vector_key_hash_map_preserves_errors_and_duplicates() {
    let config = bincode::config::standard();
    let entries = vec![(string_vector(1), 1u64), (string_vector(2), 251u64)];
    let encoded = bincode::encode_to_vec(&entries, config).unwrap();
    let mut decoded = (0..entries.len())
        .map(|index| (warmed_string_vector(index), u64::MAX))
        .collect::<HashMap<_, _>>();

    let error = bincode::decode_from_slice_into_hash_map(
    ...
    let duplicate_entries = vec![(string_vector(7), 1u64), (string_vector(7), 2)];
    let encoded = bincode::encode_to_vec(&duplicate_entries, config).unwrap();
    decoded.insert(warmed_string_vector(9), u64::MAX);
    bincode::decode_from_slice_into_hash_map(&encoded, &mut decoded, config).unwrap();
    assert_eq!(decoded.len(), 1);
    assert_eq!(decoded.get(&duplicate_entries[0].0), Some(&2));
}

#[test]
fn reusable_byte_key_hash_map_preserves_errors_and_duplicate_keys() {
    let config = bincode::config::standard();
    let entries = vec![(vec![1u8; 96], 1u64), (vec![2u8; 96], 251u64)];
    let encoded = bincode::encode_to_vec(&entries, config).unwrap();
    let mut decoded = (0..entries.len())
        .map(|index| {
            let mut key = Vec::with_capacity(192);
            key.extend_from_slice(&[index as u8; 8]);
            (key, u64::MAX)
        })
        .collect::<HashMap<_, _>>();
    ...
    let duplicate_entries = vec![(vec![7u8; 32], 1u64), (vec![7u8; 32], 2)];
    let encoded = bincode::encode_to_vec(&duplicate_entries, config).unwrap();
    decoded.insert(vec![9u8; 32], u64::MAX);
    bincode::decode_from_slice_into_hash_map(&encoded, &mut decoded, config).unwrap();
    assert_eq!(decoded.len(), 1);
    assert_eq!(decoded.get(&duplicate_entries[0].0), Some(&2));
}
...
#[test]
fn reusable_hash_set_reuses_byte_buffer_allocations() {
    let values = (0..128u64)
        .map(|index| {
            let mut value = index.to_le_bytes().to_vec();
            value
    ...
    let ordered = vec![vec![1u8; 96], vec![2u8; 96]];
    let encoded = bincode::encode_to_vec(&ordered, config).unwrap();
    decoded.insert(Vec::with_capacity(192));
    decoded.insert(vec![9u8; 8]);
    let error = bincode::decode_from_slice_into_hash_set(
        &encoded[..encoded.len() - 1],
        &mut decoded,
        config,
...
#[test]
fn reusable_hash_set_reuses_string_vector_allocations_and_preserves_errors() {
    let values = (0..64).map(string_vector).collect::<HashSet<_>>();
    let config = bincode::config::standard();
    let encoded = bincode::encode_to_vec(&values, config).unwrap();
    let mut decoded = (0..values.len())
        .map(warmed_string_vector)
        .collect::<HashSet<_>>();
    let capacity = decoded.capacity();
    let allocations = string_vector_allocations(&decoded);

    let used = bincode::decode_from_slice_into_hash_set(&encoded, &mut decoded, config).unwrap();
    assert_eq!((&decoded, used), (&values, encoded.len()));
    assert_eq!(decoded.capacity(), capacity);
    assert_eq!(string_vector_allocations(&decoded), allocations);
    ...
    let ordered = vec![string_vector(1), string_vector(2)];
    let encoded = bincode::encode_to_vec(&ordered, config).unwrap();
    decoded.insert(warmed_string_vector(8));
    decoded.insert(warmed_string_vector(9));
    let error = bincode::decode_from_slice_into_hash_set(
        &encoded[..encoded.len() - 1],
        &mut decoded,
        config,
...
impl bincode::enc::write::Writer for WriteCounter {
...
#[test]
fn boolean_batches_keep_wire_format_and_reuse_vector_storage() {
    let values = vec![true, false, true, true, false, false, true, false];
    let config = bincode::config::standard();
    let expected = [8, 1, 0, 1, 1, 0, 0, 1, 0];
    assert_eq!(bincode::encode_to_vec(&values, config).unwrap(), expected);
    assert_eq!(
        bincode::decode_from_slice::<Vec<bool>, _>(&expected, config)
            .unwrap()
            .0,
        values
    );
    assert_eq!(
        bincode::borrow_decode_from_slice::<Vec<bool>, _>(&expected, config)
            .unwrap()
            .0,
        values
    );
    ...
    decoded.extend_from_slice(&[false; 16]);
    let allocation = decoded.as_ptr();
    ...
    let array = [true, false, true, false, false, true, true, false];
    let encoded = bincode::encode_to_vec(array, config).unwrap();
    assert_eq!(encoded, [1, 0, 1, 0, 0, 1, 1, 0]);
    assert_eq!(
        bincode::decode_from_slice::<[bool; 8], _>(&encoded, config)
            .unwrap()
            .0,
        array
    );
    assert_eq!(
        bincode::borrow_decode_from_slice::<[bool; 8], _>(&encoded, config)
            .unwrap()
            .0,
        array
    );
...
#[test]
fn boolean_batches_preserve_invalid_prefix_limits_and_reader_fallback() {
    let config = bincode::config::standard();
    let mut decoded = Vec::with_capacity(16);
    decoded.extend_from_slice(&[false; 8]);
    let allocation = decoded.as_ptr();
    let error =
        bincode::decode_from_slice_into_vec(&[4, 1, 0, 2, 1], &mut decoded, config).unwrap_err();
    assert!(matches!(
        error,
    ...
    let error = bincode::decode_from_slice::<[bool; 4], _>(&[1, 0, 2, 1], config).unwrap_err();
    assert!(matches!(
        error,
        bincode::error::DecodeError::InvalidBooleanValue(2)
    ));

    let encoded = bincode::encode_to_vec(vec![true; 32], config).unwrap();
    let error =
        bincode::decode_from_slice_into_vec(&encoded, &mut decoded, config.with_limit::<16>())
            .unwrap_err();
    assert!(matches!(error, bincode::error::DecodeError::LimitExceeded));
    ...
    assert_eq!(Vec::<bool>::decode(&mut decoder).unwrap(), values);
    let encoded = bincode::encode_to_vec([true, false, true, false], config).unwrap();
    let mut decoder = bincode::de::DecoderImpl::new(NoPeek(&encoded), config, ());
    assert_eq!(
        <[bool; 4]>::decode(&mut decoder).unwrap(),
        [true, false, true, false]
    );
}

#[test]
fn signed_byte_batches_cover_vectors_arrays_configs_and_reuse() {
    let values = vec![i8::MIN, -100, -1, 0, 1, 100, i8::MAX];
    let expected = [7, 0x80, 0x9c, 0xff, 0, 1, 100, 127];

    let legacy_little = [7, 0, 0, 0, 0, 0, 0, 0, 0x80, 0x9c, 0xff, 0, 1, 100, 127];
    let legacy_big = [0, 0, 0, 0, 0, 0, 0, 7, 0x80, 0x9c, 0xff, 0, 1, 100, 127];
    macro_rules! check_config {
        ($config:expr, $expected:expr) => {{
            let config = $config;
            assert_eq!(bincode::encode_to_vec(&values, config).unwrap(), $expected);
            assert_eq!(
                bincode::decode_from_slice::<Vec<i8>, _>($expected, config)
                    .unwrap()
                    .0,
                values
            );
    ...
    check_config!(bincode::config::standard(), &expected);
    check_config!(bincode::config::standard().with_big_endian(), &expected);
    check_config!(bincode::config::legacy(), &legacy_little);
    check_config!(bincode::config::legacy().with_big_endian(), &legacy_big);

    let config = bincode::config::standard();
    let mut decoded = Vec::with_capacity(32);
    decoded.extend_from_slice(&[42; 16]);
    let allocation = decoded.as_ptr();
    bincode::decode_from_slice_into_vec(&expected, &mut decoded, config).unwrap();
    assert_eq!(decoded, values);
    assert_eq!(decoded.as_ptr(), allocation);

    let array = [i8::MIN, -1, 0, 1, i8::MAX];
    let encoded = bincode::encode_to_vec(array, config).unwrap();
    assert_eq!(encoded, [0x80, 0xff, 0, 1, 127]);
    assert_eq!(
        bincode::decode_from_slice::<[i8; 5], _>(&encoded, config)
            .unwrap()
            .0,
        array
    );
    assert_eq!(
        bincode::borrow_decode_from_slice::<[i8; 5], _>(&encoded, config)
            .unwrap()
            .0,
        array
    );
