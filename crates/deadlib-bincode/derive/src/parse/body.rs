...
impl EnumBody {
    pub(crate) fn take(input: &mut Peekable<impl Iterator<Item = TokenTree>>) -> Result<Self> {
        ...
        while stream.peek().is_some() {
            let _attributes = Attribute::try_take(stream)?;
...
impl UnnamedField {
    pub(crate) fn parse_with_name(
        ...
        loop {
            ...
            let _visibility = Visibility::try_take(input)?;
            ...
            result.push((ident, Self { r#type, attributes }));
        }
        Ok(result)
    }
    ...
    pub(crate) fn parse(
        ...
        while input.peek().is_some() {
            ...
            let r#type = read_tokens_until_punct(input, &[','])?;
            consume_punct_if(input, ',');
            result.push(Self { r#type, attributes });
        }
        Ok(result)
    }
