...
const STRINGS_PER_ENTRY: usize = 8;
...
// These are the generic reusable hash-collection implementations immediately
// before this pass. They retain buckets but drop every nested vector/string.
...
fn string_vector(index: usize) -> Vec<String> {
    (0..STRINGS_PER_ENTRY)
        .map(|part| {
            format!(
                "chart-{index:05}-part-{part:02}-{}",
                "dead-sync-rhythm".repeat(3)
            )
        })
        .collect()
}

fn string_vector_key_map() -> HashMap<Vec<String>, u64> {
    (0..ENTRY_COUNT)
        .map(|index| (string_vector(index), (index as u64).wrapping_mul(1_000_003)))
        .collect()
}

fn string_vector_value_map() -> HashMap<u64, Vec<String>> {
    (0..ENTRY_COUNT)
        .map(|index| ((index as u64).wrapping_mul(1_000_003), string_vector(index)))
        .collect()
}

fn string_vector_set() -> HashSet<Vec<String>> {
    (0..ENTRY_COUNT).map(string_vector).collect()
}

fn assert_deep_pool_churn(old: AllocStats, new: AllocStats) {
    let owned_allocations = ENTRY_COUNT * (STRINGS_PER_ENTRY + 1);
    assert!(old.allocs + old.zeroed_allocs >= owned_allocations as u64);
    assert!(old.deallocs >= owned_allocations as u64);
    ...
    V: Decode<()> + PartialEq + std::fmt::Debug,
{
    ...
    assert_deep_pool_churn(old_stats, new_stats);
    (old_stats, new_stats)
}
...
fn report_allocations(
    ...
    eprintln!(
        "eighth-pass allocation profile per operation:\n\
         string-vector map keys   old/new {key_stats:?}\n\
         string-vector map values old/new {value_stats:?}\n\
         string-vector set values old/new {set_stats:?}"
    );
}
...
fn report_cycles(encoded_keys: &[u8], encoded_values: &[u8], encoded_set: &[u8]) {
    let key_cycles = map_cycle_pair::<Vec<String>, u64>(encoded_keys, 20);
    let value_cycles = map_cycle_pair::<u64, Vec<String>>(encoded_values, 20);
    let set_cycles = set_cycle_pair::<Vec<String>>(encoded_set, 20);
    eprintln!(
        "eighth-pass thread cycles per operation:\n\
         string-vector map keys   old/new {key_cycles:?}\n\
         string-vector map values old/new {value_cycles:?}\n\
         string-vector set values old/new {set_cycles:?}"
    );
}
    ...
    V: Decode<()>,
{
    ...
    group.bench_function(BenchmarkId::new("new_deep_pool", encoded.len()), |b| {
        b.iter(|| {
...
fn eighth_pass_performance(c: &mut Criterion) {
    let config = config::standard();
    let keys = string_vector_key_map();
    let values = string_vector_value_map();
    let set = string_vector_set();
    ...
    bench_map_pair(
        c,
        "deep_reuse_string_vector_map_keys",
        &encoded_keys,
        keys.clone(),
        keys,
    );
    bench_map_pair(
        c,
        "deep_reuse_string_vector_map_values",
        &encoded_values,
        values.clone(),
        values,
    );
    bench_set_pair(
        c,
        "deep_reuse_string_vector_set_values",
        &encoded_set,
        set.clone(),
        set,
    );
...
criterion_group! {
    ...
    targets = eighth_pass_performance
}
