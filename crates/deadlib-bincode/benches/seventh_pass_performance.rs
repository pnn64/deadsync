...
// These are the reusable hash-collection fallbacks immediately before this
// pass. They retain buckets but drop every allocation owned by a byte buffer.
...
fn buffer(index: usize) -> Vec<u8> {
    let mut bytes = Vec::with_capacity(BUFFER_LEN);
    bytes.extend_from_slice(&(index as u64).to_le_bytes());
    bytes.extend((8..BUFFER_LEN).map(|offset| index.wrapping_mul(31).wrapping_add(offset) as u8));
    bytes
}

fn byte_key_map() -> HashMap<Vec<u8>, u64> {
    (0..ENTRY_COUNT)
        .map(|index| (buffer(index), (index as u64).wrapping_mul(1_000_003)))
        .collect()
}

fn byte_value_map() -> HashMap<u64, Vec<u8>> {
    (0..ENTRY_COUNT)
        .map(|index| ((index as u64).wrapping_mul(1_000_003), buffer(index)))
        .collect()
}

fn byte_set() -> HashSet<Vec<u8>> {
    (0..ENTRY_COUNT).map(buffer).collect()
}
...
fn report_allocations(
    ...
    eprintln!(
        "seventh-pass allocation profile per operation:\n\
         byte-buffer map keys   old/new {key_stats:?}\n\
         byte-buffer map values old/new {value_stats:?}\n\
         byte-buffer set values old/new {set_stats:?}"
    );
}
...
fn report_cycles(encoded_keys: &[u8], encoded_values: &[u8], encoded_set: &[u8]) {
    let key_cycles = map_cycle_pair::<Vec<u8>, u64>(encoded_keys, 25);
    let value_cycles = map_cycle_pair::<u64, Vec<u8>>(encoded_values, 25);
    let set_cycles = set_cycle_pair::<Vec<u8>>(encoded_set, 25);
    eprintln!(
        "seventh-pass thread cycles per operation:\n\
         byte-buffer map keys   old/new {key_cycles:?}\n\
         byte-buffer map values old/new {value_cycles:?}\n\
         byte-buffer set values old/new {set_cycles:?}"
    );
}
    ...
    V: Decode<()>,
{
    ...
    group.bench_function(BenchmarkId::new("new_buffer_pool", encoded.len()), |b| {
        b.iter(|| {
...
fn seventh_pass_performance(c: &mut Criterion) {
    let config = config::standard();
    let keys = byte_key_map();
    let values = byte_value_map();
    let set = byte_set();
    ...
    bench_map_pair(
        c,
        "deep_reuse_byte_buffer_map_keys",
        &encoded_keys,
        keys.clone(),
        keys,
    );
    bench_map_pair(
        c,
        "deep_reuse_byte_buffer_map_values",
        &encoded_values,
        values.clone(),
        values,
    );
    bench_set_pair(
        c,
        "deep_reuse_byte_buffer_set_values",
        &encoded_set,
        set.clone(),
        set,
    );
...
criterion_group! {
    ...
    targets = seventh_pass_performance
}
