use deadlib_render_backend_vulkan::bench_support;
...
fn main() {
    let descriptors = bench_support::descriptor_events(EVENTS);
    print_pair(
        "Vulkan descriptor binding retention",
        descriptors.len(),
        &measure(2_048, || {
            bench_support::descriptors_old(black_box(&descriptors))
        }),
        &measure(2_048, || {
            bench_support::descriptors_new(black_box(&descriptors))
        }),
    );

    let owner = Arc::new(0x5a5a_a5a5_1357_2468_u64);
    print_pair(
        "Vulkan geometry device ownership",
        EVENTS,
        &measure(2_048, || {
            bench_support::owner_refs_old(black_box(&owner), EVENTS)
        }),
        &measure(2_048, || {
            bench_support::owner_refs_new(black_box(&owner), EVENTS)
        }),
    );

    let vertices = bench_support::vertex_events(EVENTS);
    print_pair(
        "Vulkan draw vertex accounting",
        vertices.len(),
