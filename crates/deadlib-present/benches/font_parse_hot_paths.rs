...
fn assert_improved(
    ...
    old: &BenchResult,
    new: &BenchResult,
    expect_allocation_reduction: bool,
) {
    ...
    if expect_allocation_reduction {
        assert!(
        ...
        assert!(
            new.allocated.allocs <= old.allocated.allocs,
            "{label} increased allocations"
        );
        assert!(
            new.allocated.churn() <= old.allocated.churn(),
            "{label} increased allocation churn"
        );
    }
}
...
fn run_pair(
    ...
    assert_improved(label, &old, &new, expect_allocation_reduction);
}

fn main() {
    let values = (0..ITEMS)
        .map(|index| format!("Setting{index:05} = value-{index:05}-doubleres"))
        .collect::<Vec<_>>();
    run_pair(
        "borrowed normalized INI values",
        || ini_values_old(black_box(&values)),
        || ini_values_new(black_box(&values)),
        true,
    );
    ...
    let mappings = (0..ITEMS as u32)
        .map(|ch| (0x20 + ch, ch as usize % 256))
        .chain([(0x10_FFFF, 512)])
        .collect::<Vec<_>>();
    run_pair(
        "direct bounded glyph dispatch",
        || glyph_dispatch_old(black_box(&mappings), 256),
        || glyph_dispatch_new(black_box(&mappings), 256),
        false,
    );
}
