use deadlib_present::font::bench_support::{
    import_map_merge_new, import_map_merge_old, import_paths_new, import_paths_old,
    import_specs_new, import_specs_old,
};
...
fn main() {
    let raw_imports = (0..ITEMS)
        .map(|index| format!(" Group{}/Font{index:05} ", index % 16))
        .collect::<Vec<_>>()
        .join(",,");
    run_pair(
        "borrowed import specs",
        || import_specs_old(black_box(&raw_imports), true),
        || import_specs_new(black_box(&raw_imports), true),
    );

    let base_inis = (0..ITEMS)
        .map(|index| PathBuf::from(format!("Themes/Simply Love/Fonts/Font{index:05}.ini")))
        .collect::<Vec<_>>();
    let specs = (0..ITEMS)
        .map(|index| format!("Shared/Import{index:05}"))
        .collect::<Vec<_>>();
    run_pair(
        "deduplicated import candidates",
        || import_paths_old(black_box(&base_inis), black_box(&specs)),
        || import_paths_new(black_box(&base_inis), black_box(&specs)),
    );

    let entries = (0..ITEMS as u64)
        .map(|key| (key, key.wrapping_mul(0x9E37_79B9_7F4A_7C15)))
        .collect::<Vec<_>>();
    run_pair(
        "size-aware imported map merge",
        || import_map_merge_old(black_box(&entries)),
        || import_map_merge_new(black_box(&entries)),
    );
}
