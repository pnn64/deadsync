...
/// Which screen corner or center seam the overlay anchors to.
...
impl FrameStatsSample {
    ...
    /// Idle headroom: the slice of the frame interval not spent in a measured phase.
    #[inline(always)]
    pub const fn idle_us(&self) -> u32 {
        self.frame_us.saturating_sub(self.measured_us())
    }

    /// CPU work this frame: every measured phase except the GPU/swapchain wait.
    #[inline(always)]
...
/// Precomputed sync-health readouts supplied by the shell.
#[derive(Clone, Copy, Debug)]
pub struct FrameStatsSummary {
    ...
    /// Smoothed jitter of the frame interval, in microseconds.
    pub frame_jitter_us: u32,
    /// Smoothed jitter of the display-clock error, in microseconds.
    pub display_error_jitter_us: u32,
    /// Slow-decay worst-recent-frame hold, in microseconds.
    pub spike_hold_us: u32,
    /// Monitor target frame time for graph reference lines, in microseconds (0 = none).
    pub target_frame_us: u32,
    /// Smoothed CPU work per frame, excluding GPU wait, in microseconds.
    pub cpu_work_us: u32,
    /// Smoothed GPU/swapchain wait per frame, in microseconds.
    pub gpu_wait_us: u32,
    /// Frames in the recent ring window that exceeded the stutter threshold.
