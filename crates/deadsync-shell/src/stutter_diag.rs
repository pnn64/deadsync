...
pub fn stutter_diag_dump_lines(
    ...
    for sample in frames {
        ...
        lines.push(format!(
            "Stutter recorder frame age_ms={:.3} screen={:?} dt_ms={:.3} expected_ms={:.3} x{:.2} req={} phases_ms=[pre:{:.3} rq:{:.3} in:{:.3} up:{:.3} comp:{:.3} upload:{:.3} draw:{:.3}] draw_ms=[acq:{:.3} sub:{:.3} present:{:.3} gpu_wait:{:.3} setup:{:.3} draw_prep:{:.3} upload:{:.3} prepare:{:.3} record:{:.3}] draw_dbg=[submission:[render_list:{} direct_frame:{} prep_bypass:{}] cached_miss:{}] display=[err_ms:{:+.3} catch:{}] present=[mode:{} display:{} host:{} inflight:{} wait:{} back:{} idle:{} subopt:{}]",
            age_ms,
            sample.screen,
            ...
            sample.render_list_submissions,
            sample.direct_frame_submissions,
            u8::from(sample.draw_prep_bypassed),
...
impl StutterDiagFrameSample {
    const fn empty() -> Self {
        Self {
            ...
            draw_prepare_us: 0,
            draw_record_us: 0,
            render_list_submissions: 0,
            direct_frame_submissions: 0,
            draw_prep_bypassed: false,
...
impl StutterDiagRecorder {
    ...
    pub fn record_frame(
        ...
        self.frames.push(StutterDiagFrameSample {
            ...
            render_list_submissions: draw_stats.render_list_submissions,
            direct_frame_submissions: draw_stats.direct_frame_submissions,
            draw_prep_bypassed: draw_stats.draw_prep_bypassed,
...
mod tests {
    ...
    fn dump_lines_preserve_frame_display_and_audio_diagnostics() {
        ...
        assert!(lines[1].contains(
            "draw_dbg=[submission:[render_list:0 direct_frame:1 prep_bypass:1] cached_miss:3]"
        ));
