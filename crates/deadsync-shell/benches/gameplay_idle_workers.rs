...
fn print_result(label: &str, result: &BenchResult) {
    ...
    println!(
        "{label:<22} {:>10.2} ns/frame  {:>10.2} cycles/frame  {:>10.2} worst ns  \
...
fn main() {
    ...
    compare(
        "idle QR-login maintenance",
        measure(|| old.legacy_qr_frame()),
        measure(|| new.gated_qr_frame()),
    );

    let mut old = GameplayIdleWorkersBenchmark::default();
    let mut new = GameplayIdleWorkersBenchmark::default();
    compare(
        "idle score-import maintenance",
        measure(|| old.legacy_score_frame()),
        measure(|| new.gated_score_frame()),
    );
