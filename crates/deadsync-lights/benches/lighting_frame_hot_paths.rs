use deadsync_lights::{HideFlags, bench_support};
...
fn print_pair(title: &str, old: &Row, new: &Row) {
    ...
    assert_eq!(new.alloc.churn(), 0, "{title} new path churned memory");
    println!("\n{title} ({CASES} states/op)");
    print_row("old", old);
    print_row("new", new);
    println!(
...
fn print_row(label: &str, row: &Row) {
    println!(
        "  {label:<3} {:>11.0} ns/op  {:>11.0} cycles/op  {:>11.0} p95 ns  \
         {:>8.2} Mstate/s  {:>5} alloc  {:>5} realloc  {:>5} free  {:>10} churn B",
        row.median_ns,
...
fn throughput(row: &Row) -> f64 {
    CASES as f64 * 1e9 / row.median_ns
}
...
fn main() {
    let patterns = state_patterns();
    let state_fixture = bench_support::state_fixture(black_box(&patterns));
    println!(
        "light State footprint: {} B old -> {} B new ({:.1}% smaller)",
        bench_support::legacy_state_size(),
        bench_support::state_size(),
        (1.0 - bench_support::state_size() as f64 / bench_support::legacy_state_size() as f64)
            * 100.0,
    );
    print_pair(
        "light-state change detection",
        &measure(OPERATIONS, || {
            bench_support::state_scan_old(black_box(&state_fixture))
        }),
        &measure(OPERATIONS, || {
            bench_support::state_scan_new(black_box(&state_fixture))
        }),
    );

    let pressed_patterns = pressed_patterns();
    let pressed_fixture = bench_support::pressed_fixture(black_box(&pressed_patterns));
    print_pair(
        "physical-button merge and output scan",
        &measure(OPERATIONS, || {
            bench_support::pressed_merge_old(black_box(&pressed_fixture))
        }),
        &measure(OPERATIONS, || {
            bench_support::pressed_merge_new(black_box(&pressed_fixture))
        }),
    );

    let hide_cases = hide_cases();
    let hide_fixture = bench_support::hide_fixture(black_box(&hide_cases));
    print_pair(
        "cabinet hide-policy scan",
        &measure(OPERATIONS, || {
            bench_support::hide_scan_old(black_box(&hide_fixture))
        }),
        &measure(OPERATIONS, || {
            bench_support::hide_scan_new(black_box(&hide_fixture))
        }),
    );
}

fn state_patterns() -> Vec<u32> {
...
fn pressed_patterns() -> Vec<(u16, u16, bool)> {
    let mut seed = 0x2468_ace1_u32;
    (0..CASES)
        .map(|case| {
            seed = seed.wrapping_mul(1_664_525).wrapping_add(1_013_904_223);
            let buttons = ((seed & 0x3f) | (((seed >> 6) & 0x3f) << 8)) as u16;
            let menu = (((seed >> 12) & 0x3f) | (((seed >> 18) & 0x3f) << 8)) as u16;
            (buttons, menu, case % 3 == 0)
        })
        .collect()
}

fn hide_cases() -> Vec<([bool; 2], [HideFlags; 2])> {
    let mut seed = 0xdead_beef_u32;
    (0..CASES)
        .map(|_| {
            seed = seed.wrapping_mul(1_664_525).wrapping_add(1_013_904_223);
            let flags = |bits: u32| HideFlags {
                all: bits & 1 != 0,
                marquee: bits & 2 != 0,
                bass: bits & 4 != 0,
            };
