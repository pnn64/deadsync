use deadsync_noteskin::actor::{element_contains_hint, element_contains_hint_legacy_for_bench};
...
const CASES: [(&str, &str); 8] = [
    ("Down Hold Explosion", "hold explosion"),
    ("Left Roll Explosion", "ROLL"),
    ("Up Tap Note", "hold explosion"),
    ("Down Receptor Waiting", "RECEPTOR"),
    ("Fallback Explosion Dim", "explosion"),
    ("Döwn Hold Explosion", "DÖWN"),
    ("Very Long Noteskin Element Name", "element name"),
    ("Empty Hint Matches", ""),
];
...
fn main() {
    ...
    let old = measure(element_contains_hint_legacy_for_bench);
    let new = measure(element_contains_hint);
    assert_eq!(old.checksum, new.checksum);

    println!(
        "noteskin element hint matching ({} cases x {RUNS} runs)",
        CASES.len()
    );
...
fn measure(matches: fn(&str, &str) -> bool) -> BenchResult {
    for _ in 0..1_000 {
        black_box(batch_checksum(matches));
    }
    ...
    for run in 0..RUNS {
        checksum = checksum.rotate_left(7) ^ black_box(batch_checksum(matches)) ^ run as u64;
    }
    BenchResult {
...
fn batch_checksum(matches: fn(&str, &str) -> bool) -> u64 {
    CASES.iter().fold(0_u64, |checksum, (element, hint)| {
        checksum.rotate_left(3) ^ u64::from(matches(black_box(element), black_box(hint)))
    })
}
