use deadsync_noteskin::itg::ItgLookupBench;
...
type LookupBatch = fn(&ItgLookupBench) -> u64;

fn main() {
    let lookups = ItgLookupBench::new();
    assert_eq!(
        lookups.legacy_ini_checksum(),
        lookups.current_ini_checksum()
    );
    assert_eq!(
        lookups.legacy_path_checksum(),
        lookups.current_path_checksum()
    );

    run_case(
        "ITG noteskin INI metric lookup",
        &lookups,
        ItgLookupBench::legacy_ini_checksum,
        ItgLookupBench::current_ini_checksum,
    );
    run_case(
        "ITG noteskin filesystem cache hit",
        &lookups,
        ItgLookupBench::legacy_path_checksum,
        ItgLookupBench::current_path_checksum,
    );
}

fn run_case(label: &str, lookups: &ItgLookupBench, legacy: LookupBatch, current: LookupBatch) {
    let old = measure(lookups, legacy);
    let new = measure(lookups, current);
    assert_eq!(old.checksum, new.checksum);

    println!("{label} ({LOOKUPS_PER_RUN} cases x {RUNS} runs)");
...
fn measure(lookups: &ItgLookupBench, lookup: LookupBatch) -> BenchResult {
    for _ in 0..1_000 {
        black_box(lookup(black_box(lookups)));
    }
    ...
    for run in 0..RUNS {
        checksum = checksum.rotate_left(7) ^ black_box(lookup(black_box(lookups))) ^ run as u64;
    }
    BenchResult {
...
fn print_result(label: &str, result: &BenchResult) {
    let operations = (LOOKUPS_PER_RUN * RUNS) as f64;
    println!(
