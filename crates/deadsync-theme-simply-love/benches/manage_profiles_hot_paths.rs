...
use deadsync_theme_simply_love::screens::manage_local_profiles::ManageProfilesHotBenchmark;
...
fn main() {
    ...
    let (old_name, new_name) = sample_pair(
        || measure(|| benchmark.legacy_name_frame(0.25)),
        || measure(|| benchmark.prepared_name_frame(0.25)),
    );
    report_pair("name-entry cursor text", 1, &old_name, &new_name);

    let (old_picker, new_picker) = sample_pair(
        || measure(|| benchmark.legacy_picker_frame()),
        || measure(|| benchmark.prepared_picker_frame()),
    );
    report_pair("ITG profile picker text", 15, &old_picker, &new_picker);

    let (old_message, new_message) = sample_pair(
        || measure(|| benchmark.legacy_message_frame()),
        || measure(|| benchmark.prepared_message_frame()),
    );
    report_pair("import summary text", 12, &old_message, &new_message);
}
