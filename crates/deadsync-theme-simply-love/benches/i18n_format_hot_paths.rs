...
fn run_fixture(
    template: &str,
    args: &[(&str, &str)],
    mut format: impl FnMut(&str, &[(&str, &str)]) -> Arc<str>,
) -> u64 {
    let mut result = 0u64;
    for _ in 0..REPEATS {
        let text = format(black_box(template), black_box(args));
        result = result
            .rotate_left(7)
            .wrapping_add(checksum(black_box(text.as_ref())));
    }
    result
}
...
fn bench_case(title: &str, template: &str, args: &[(&str, &str)], expected_new_allocs: u64) {
    let (old, new) = measure_pair(
        || run_fixture(template, args, benchmark_format_translation_reference),
        || run_fixture(template, args, benchmark_format_translation_current),
    );
    assert_eq!(old.checksum, new.checksum, "{title}: behavior diverged");

    println!("\n{title} ({REPEATS} formats/sample)");
...
fn main() {
    bench_case(
        "stack-backed short translation",
        "Ready {player}",
        &[("player", "P1")],
        REPEATS as u64,
    );
    ...
    bench_case(
        "single-pass multi-argument substitution",
        "{processed}/{total} cached:{cached} ready:{ready} below:{below} threshold:{threshold} unchanged:{unchanged} failed:{failed}",
        &[
            ("processed", "128"),
            ("total", "512"),
            ("cached", "64"),
    ...
    let repeated_template = "{a}|{b}|{c}|{d}|{e}|{f}|{g}|{h}|".repeat(16);
    bench_case(
        "exact capacity for repeated expansion",
        repeated_template.as_str(),
        &[
            ("a", "expanded-placeholder-a"),
            ("b", "expanded-placeholder-b"),
            ("c", "expanded-placeholder-c"),
            ("d", "expanded-placeholder-d"),
            ("e", "expanded-placeholder-e"),
            ("f", "expanded-placeholder-f"),
            ("g", "expanded-placeholder-g"),
            ("h", "expanded-placeholder-h"),
        ],
        (REPEATS * 2) as u64,
    );
}
