...
fn main() {
    ...
    let (old_identity, new_identity) = sample_pair(
        || measure(|| benchmark.legacy_identity()),
        || measure(|| benchmark.shared_identity()),
    );
    report_pair(
        "profile name + avatar actor ownership",
        2,
        &old_identity,
        &new_identity,
    );

    let (old_mods, new_mods) = sample_pair(
        || measure(|| benchmark.legacy_recent_mods()),
        || measure(|| benchmark.prepared_recent_mods()),
    );
    report_pair("selected-profile modifier text", 1, &old_mods, &new_mods);

    let mut old_actors = Vec::with_capacity(56);
    let mut new_actors = Vec::with_capacity(56);
    let (old_staging, new_staging) = sample_pair(
        || measure(|| benchmark.legacy_actor_staging(&mut old_actors)),
