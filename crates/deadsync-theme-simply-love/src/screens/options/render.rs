use super::text_wrap::wrap_text_with_measure;
use super::*;
    ...
    asset_manager
        .with_fonts(|all_fonts| {
            asset_manager.with_font("miso", |miso_font| {
                wrap_text_with_measure(raw_text, max_width_px, |text| {
                    font::measure_line_width_logical(miso_font, text, all_fonts) as f32 * zoom
                })
            })
        })
...
pub fn push_actors(
    ...
    // --- global colors ---
    const COL_ACTIVE_BG: [f32; 4] = deadlib_present::color::rgba_hex("#333333"); // active bg for normal rows

    // inactive bg = #071016 @ 0.8 alpha
    const BASE_INACTIVE: [f32; 4] = deadlib_present::color::rgba_hex("#071016");
    ...
    match state.view {
        ...
        OptionsView::Submenu(kind) => {
            ...
            if is_launcher_submenu(kind) {
                ...
                const SL_GRAY: [f32; 4] = deadlib_present::color::rgba_hex("#808080");
                let sl_gray = SL_GRAY;
