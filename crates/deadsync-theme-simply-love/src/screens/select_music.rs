...
#[inline(always)]
fn music_wheel_hold_spin_speed(configured: u8) -> f32 {
    if configured == 0 {
...
struct FolderStatsCache {
    ...
    pack_by_key: FxHashMap<Arc<str>, usize>,
...
pub struct State {
    /// Reused visible-wheel buffer owned by Select Music's main-thread state.
    /// It lives for the screen, is pre-sized to the largest built-in cached sort
    /// during initialization, and is then cleared/refilled without allocation.
    ...
    /// Immutable sort views owned by Select Music on the main thread and shared
    ...
    title_entries: Arc<[MusicWheelEntry]>,
    artist_entries: Arc<[MusicWheelEntry]>,
    genre_entries: Arc<[MusicWheelEntry]>,
    bpm_entries: Arc<[MusicWheelEntry]>,
    length_entries: Arc<[MusicWheelEntry]>,
    meter_entries: Arc<[MusicWheelEntry]>,
    popularity_entries: Arc<[MusicWheelEntry]>,
    recent_entries: Arc<[MusicWheelEntry]>,
    top_grades_entries: Arc<[MusicWheelEntry]>,
    popularity_p1_entries: Arc<[MusicWheelEntry]>,
    popularity_p2_entries: Arc<[MusicWheelEntry]>,
    recent_p1_entries: Arc<[MusicWheelEntry]>,
    recent_p2_entries: Arc<[MusicWheelEntry]>,
    top_grades_p1_entries: Arc<[MusicWheelEntry]>,
    top_grades_p2_entries: Arc<[MusicWheelEntry]>,
    /// Union of the joined players' favorites, used only to decide whether the
    /// Favorites menu item is available. The active wheel is built per-player.
    joined_favorites_entries: Arc<[MusicWheelEntry]>,
    playlist_entries: Arc<[MusicWheelEntry]>,
...
fn history_score(
    ...
    chart_hash: &str,
) -> Option<score_data::CachedScore> {
    history
        .cached_scores
        .binary_search_by(|(hash, _)| hash.as_str().cmp(chart_hash))
        .ok()
...
fn song_has_cached_score(
    ...
    for side in [profile_data::PlayerSide::P1, profile_data::PlayerSide::P2] {
        ...
        for chart in &song.charts {
            if history_score(side_history, &chart.short_hash).is_some_and(cached_score_exists) {
                return true;
            }
        }
    }
...
fn sync_new_pack_names(
    ...
    scored_pack_names: &FxHashSet<String>,
    mode: NewPackMode,
    profiles: &SelectMusicProfileView,
    known_packs: &mut profile_data::KnownPackSnapshot,
) -> (FxHashSet<String>, Vec<crate::SimplyLoveProfileRequest>) {
    match mode {
    ...
    for entry in all_entries {
        match entry {
            MusicWheelEntry::PackHeader {
                ...
                if !show_only_active_series_header || expanded_series_name == Some(name.as_ref()) {
            ...
            MusicWheelEntry::Song(_) => {
                if current_pack_visible && expanded_pack_name == current_pack_key {
                    entries.push(entry.clone());
                }
            }
        }
...
fn group_name_for_song(
    ...
    let mut current_pack_key: Option<&Arc<str>> = None;
    for entry in grouped_entries {
        match entry {
            ...
            MusicWheelEntry::PackHeader { name, pack_key, .. } => {
                current_pack_key = Some(pack_key.as_ref().unwrap_or(name));
            }
            MusicWheelEntry::Song(song) => {
                if Arc::ptr_eq(song, target_song) {
                    return current_pack_key.map(Arc::clone);
                }
            }
        }
    }
...
fn series_name_for_song(
    ...
    let mut current_series: Option<&Arc<str>> = None;
    for entry in series_entries {
        match entry {
            MusicWheelEntry::PackHeader {
                ...
                parent_series,
                ..
            } => current_series = parent_series.as_ref(),
            MusicWheelEntry::PackHeader {
                parent_series: None,
                ..
            } => current_series = None,
            ...
            MusicWheelEntry::Song(song) if Arc::ptr_eq(song, target_song) => {
                return current_series.map(Arc::clone);
            }
            MusicWheelEntry::Song(_) => {}
        }
    }
...
fn series_name_for_pack<'a>(
    ...
    series_entries.iter().find_map(|entry| match entry {
        MusicWheelEntry::PackHeader {
            ...
            parent_series,
            ..
        } if key.as_ref() == pack_key => parent_series.as_ref(),
        _ => None,
    })
}
...
#[inline(always)]
fn first_header_name(entries: &[MusicWheelEntry]) -> Option<Arc<str>> {
...
fn push_series_pack(
    ...
    {
        *parent = parent_series.map(Arc::from);
    }
...
fn grouped_song_entries(groups: Vec<GroupedSongs>) -> Vec<MusicWheelEntry> {
    ...
    for (header_idx, group) in groups.into_iter().enumerate() {
        entries.push(MusicWheelEntry::PackHeader {
            name: Arc::from(song_sort_group_label(&group.group)),
    ...
    let pack_is_favorited = |pack_key: &str| -> bool {
        (p1_joined && favorites.pack_names[0].contains(pack_key))
            ...
            || series_name_for_pack(series_entries, pack_key).is_some_and(|series| {
                (p1_joined && favorites.series_names[0].contains(series.as_ref()))
                    || (p2_joined && favorites.series_names[1].contains(series.as_ref()))
            })
    };
    ...
    let pack_is_favorited = |pack_key: &str| {
        favorites.pack_names[side_index].contains(pack_key)
            || series_name_for_pack(series_entries, pack_key)
                .is_some_and(|series| favorites.series_names[side_index].contains(series.as_ref()))
    };
    let song_is_favorited = |song: &SongData| {
        song.charts
            .iter()
            .any(|chart| favorites.chart_hashes[side_index].contains(&chart.short_hash))
    };
...
fn apply_wheel_sort(state: &mut State, sort_mode: WheelSortMode) {
    // Favorites are mutable while this sort is active, so selecting the same
    // player's Favorites again must still rebuild the wheel.
    if state.sort_mode == sort_mode && !matches!(sort_mode, WheelSortMode::Favorites(_)) {
        return;
    }
    ...
    match sort_mode {
        WheelSortMode::Series => {
            state.all_entries = Arc::clone(&state.series_entries);
        ...
        WheelSortMode::Group => {
            state.all_entries = Arc::clone(&state.group_entries);
            state.expanded_pack_name = selected_song
                .as_ref()
        ...
        WheelSortMode::Title => {
            state.all_entries = Arc::clone(&state.title_entries);
            state.expanded_pack_name = selected_song
                .as_ref()
        ...
        WheelSortMode::Artist => {
            state.all_entries = Arc::clone(&state.artist_entries);
            state.expanded_pack_name = selected_song
                .as_ref()
        ...
        WheelSortMode::Genre => {
            state.all_entries = Arc::clone(&state.genre_entries);
            state.expanded_pack_name = selected_song
                .as_ref()
        ...
        WheelSortMode::Bpm => {
            state.all_entries = Arc::clone(&state.bpm_entries);
            state.expanded_pack_name = selected_song
                .as_ref()
        ...
        WheelSortMode::Length => {
            state.all_entries = Arc::clone(&state.length_entries);
            state.expanded_pack_name = selected_song
                .as_ref()
        ...
        WheelSortMode::Meter => {
            state.all_entries = Arc::clone(&state.meter_entries);
            state.expanded_pack_name = selected_song
                .as_ref()
        ...
        WheelSortMode::Popularity => {
            state.all_entries = Arc::clone(&state.popularity_entries);
            state.expanded_pack_name = selected_song
                .as_ref()
        ...
        WheelSortMode::Recent => {
            state.all_entries = Arc::clone(&state.recent_entries);
            state.expanded_pack_name = selected_song
                .as_ref()
        ...
        WheelSortMode::TopGrades => {
            state.all_entries = Arc::clone(&state.top_grades_entries);
            state.expanded_pack_name = selected_song
                .as_ref()
        ...
        WheelSortMode::TopGradesP2 => {
            state.all_entries = Arc::clone(&state.top_grades_p2_entries);
            state.expanded_pack_name = selected_song
                .as_ref()
        ...
        WheelSortMode::Favorites(side) => {
            ...
            state.expanded_pack_name = selected_song
                ...
                .or_else(|| first_header_name(&state.all_entries));
        }
        WheelSortMode::Playlist => {
            if let Some(playlist_entries) = state
                ...
                .map(|playlist| Arc::clone(&playlist.entries))
            {
                state.playlist_entries = playlist_entries;
                state.all_entries = Arc::clone(&state.playlist_entries);
                state.expanded_pack_name = selected_song
                    .as_ref()
...
pub fn init(init_view: SelectMusicInitView) -> State {
    ...
    let mut last_pack_key: Option<Arc<str>> = None;
    ...
    for (i, pack) in song_cache.iter().enumerate() {
        let mut pack_key: Option<String> = None;
        ...
        for song in &pack.songs {
            ...
            let pack_key = pack_key.get_or_insert_with(|| {
                matched_packs += 1;
                let name = pack.name.clone();
                let key = pack.group_name.clone();
                    ...
                    .is_some_and(|p| p.to_string_lossy() == last_path)
            {
                ...
                last_pack_key = Some(Arc::from(pack_key.as_str()));
            }
        }
...
pub fn init_placeholder() -> State {
    ...
    State {
        all_entries: Arc::from([]),
        series_entries: Arc::from([]),
        group_entries: Arc::from([]),
        folder_stats: FolderStatsCache::default(),
        title_entries: Arc::from([]),
        artist_entries: Arc::from([]),
        genre_entries: Arc::from([]),
        bpm_entries: Arc::from([]),
        length_entries: Arc::from([]),
        meter_entries: Arc::from([]),
        popularity_entries: Arc::from([]),
        recent_entries: Arc::from([]),
        top_grades_entries: Arc::from([]),
        popularity_p1_entries: Arc::from([]),
        popularity_p2_entries: Arc::from([]),
        recent_p1_entries: Arc::from([]),
        recent_p2_entries: Arc::from([]),
        top_grades_p1_entries: Arc::from([]),
        top_grades_p2_entries: Arc::from([]),
        joined_favorites_entries: Arc::from([]),
        playlist_entries: Arc::from([]),
...
fn maybe_prewarm_replaygain_for_pack(
    ...
    if state.last_replaygain_prewarmed_pack.as_deref() == Some(pack.as_ref()) {
        return None;
    }
    let mut current_pack_key: Option<&str> = None;
...
/// Holding Select turns MenuLeft/MenuRight into difficulty steps for that
/// player instead of wheel movement; pad Left/Right are deliberately left
/// alone so a dance-pad player can't step difficulty by accident. Consumes
/// the press (returns true) whenever Select is held, even with no song or no
/// adjacent chart, so the wheel never moves while the modifier is down.
#[inline(always)]
...
fn build_select_music_menu(state: &State) -> select_music_menu::MenuLists {
    ...
    let any_has_favorites = state.joined_favorites_entries.len() > 1;
...
fn refresh_after_reload(state: &mut State, song_packs: Vec<SongPack>) {
    ...
    let selected_pack_key = if let Some(song) = selected_song.as_ref() {
        ...
        && refreshed.all_entries.iter().any(|entry| {
            matches!(
                ...
                MusicWheelEntry::PackHeader {
                    ...
                    parent_series: Some(_),
                    ..
                } if name.as_ref() == expanded.as_ref()
            )
        })
    {
    ...
    if let Some(expanded) = expanded_pack_name
        && refreshed
            .all_entries
            .iter()
            .any(|entry| entry.section_key() == Some(expanded.as_ref()))
    {
    ...
    if let Some(simfile_path) = selected_simfile_path {
        if let Some(index) = refreshed.entries.iter().position(|entry| {
            ...
            restored = true;
        } else if let Some(pack_key) = selected_pack_key.as_ref()
            && refreshed.expanded_pack_name.as_deref() != Some(pack_key.as_ref())
            && refreshed
                .all_entries
                .iter()
                .any(|entry| entry.section_key() == Some(pack_key.as_ref()))
        {
            refreshed.expanded_pack_name = Some(pack_key.clone());
            rebuild_displayed_entries(&mut refreshed);
            if let Some(index) = refreshed.entries.iter().position(|entry| {
                matches!(entry, MusicWheelEntry::Song(song) if song.simfile_path == simfile_path)
            }) {
    ...
    if !restored
        && let Some(pack_key) = selected_pack_key
        && let Some(index) = refreshed
            .entries
            .iter()
            .position(|entry| entry.section_key() == Some(pack_key.as_ref()))
    {
        refreshed.selected_index = index;
    }
...
fn selected_pack_sync_paths(state: &State, target_pack_key: &str) -> Vec<PathBuf> {
    let mut current_pack_key: Option<&str> = None;
...
fn show_sync_pack_overlay(state: &mut State) {
    let Some(MusicWheelEntry::PackHeader { name, pack_key, .. }) =
        state.entries.get(state.selected_index)
    else {
        return;
    };
...
fn handle_profile_switch_overlay_input(
    state: &mut State,
    ev: &InputEvent,
    effects: &mut Vec<ThemeEffect>,
) -> ThemeEffect {
    ...
    let outcome = profile_boxes::handle_input(overlay, ev, effects);
    match outcome {
        profile_boxes::InputOutcome::Selected => {
            state.profile_switch_overlay = None;
            state.profile_switch_overlay_is_late_join = false;
        }
        profile_boxes::InputOutcome::Cancelled(_) => {
        ...
        profile_boxes::InputOutcome::Continue => {}
    }
    ThemeEffect::None
}
...
fn dispatch_menu_action(
    ...
    match action {
        ...
        select_music_menu::Action::SortByFavorites => {
            apply_wheel_sort(state, WheelSortMode::Favorites(side));
            hide_select_music_menu(state);
            ThemeEffect::None
        }
...
fn collapse_expanded_pack(state: &mut State, pack: Arc<str>) {
    ...
    if let Some(new_sel) = state
        .entries
        .iter()
        .position(|entry| entry.section_key() == Some(pack.as_ref()))
    {
...
fn handle_confirm_impl(state: &mut State) -> ThemeEffect {
    ...
    match state.entries.get(state.selected_index).cloned() {
        ...
        Some(entry @ MusicWheelEntry::PackHeader { .. }) if entry.is_series_header() => {
            ...
            queue_sfx(state, "assets/sounds/expand.ogg");
            if state.expanded_series_name.as_deref() == Some(name.as_ref()) {
                state.expanded_series_name = None;
            } else {
                state.expanded_series_name = Some(Arc::clone(&name));
            }
        ...
        Some(MusicWheelEntry::PackHeader {
            ...
            let target = pack_key
                .as_ref()
                .map_or_else(|| Arc::clone(&name), Arc::clone);
            if state.policy.interaction.new_pack_mode == NewPackMode::OpenPack
                && state.new_pack_names.remove(target.as_ref())
            {
                ...
                if !profile_ids.is_empty() {
                    queue_profile(
                        ...
                        crate::SimplyLoveProfileRequest::MarkPacksKnown {
                            profile_ids,
                            pack_names: vec![target.to_string()],
                        },
                    );
                }
            }
            if state.expanded_pack_name.as_deref() == Some(target.as_ref()) {
                state.expanded_pack_name = None;
            } else {
                ...
                state.expanded_pack_name = Some(Arc::clone(&target));
            }
            rebuild_displayed_entries(state);
            if let Some(new_sel) = state
                .entries
                .iter()
                .position(|entry| entry.section_key() == Some(target.as_ref()))
            {
                state.selected_index = new_sel;
            } else {
                state.selected_index = 0;
...
fn configurable_shortcut_action(
    ...
    let shortcuts = state.policy.interaction;
    let code = key.code;
    if code == shortcuts.practice_shortcut {
    ...
    if code == shortcuts.song_search_shortcut {
        return Some(select_music_menu::Action::SongSearch);
    }
    if code == shortcuts.reload_shortcut {
        return Some(select_music_menu::Action::ReloadSongsCourses);
    }
    if code == shortcuts.test_input_shortcut {
...
fn handle_input_impl(
    ...
    if state.profile_switch_overlay.is_some() {
        return handle_profile_switch_overlay_input(state, ev, effects);
    }
...
fn update_impl(state: &mut State, dt: f32, smx: &SmxAssignmentView) -> ThemeEffect {
    ...
    maybe_clear_selected_pack_on_score(state, state.policy.interaction.new_pack_mode);
    ...
    let new_cdtitle = if state.policy.media.show_cdtitles {
        selected_song
            .as_deref()
            .and_then(|song| song.cdtitle_path.as_deref())
    } else {
        None
    };
    ...
    // --- Delayed Updates ---
    if state.policy.media.show_previews
        && !state.preview_music_muted
        && allow_gs_fetch_for_selection(state)
    {
        sync_preview_song(
...
fn test_media_song(index: usize) -> Arc<SongData> {
    Arc::new(SongData {
        ...
        background_path: Some(PathBuf::from(format!(
            "Songs/Bench/Song{index}/background.png"
        ))),
        ...
        has_lua: false,
        cdtitle_path: Some(PathBuf::from(format!(
            "Songs/Bench/Song{index}/cdtitle.png"
        ))),
        music_path: None,
...
pub fn push_actors(
    ...
    let step_artist_expanded = presentation.step_artist_expanded && !is_versus;
    ...
    let push_pane = |out: &mut Vec<Actor>,
        ...
        let show_rivals = gs_active && presentation.show_scorebox && scorebox_cycle_enabled;
    ...
    if presentation.show_scorebox && scorebox_cycle_enabled {
        ...
        let mut push_scorebox = |side: profile_data::PlayerSide,
                                 ...
                                 z_boost: i16| {
            let chart_hash =
                if allow_gs_fetch && presentation.show_scorebox && scorebox_cycle_enabled {
...
mod tests {
    ...
    fn embedded_profile_consumer_preserves_sound_before_selection() {
        let mut state = init_placeholder();
        let mut overlay = profile_boxes::init(ProfilePickerView::default());
        profile_boxes::set_fast_switch(&mut overlay, true);
        profile_boxes::set_joined(&mut overlay, true, false);
        state.profile_switch_overlay = Some(overlay);
        let mut effects = Vec::with_capacity(2);

        let effect = handle_profile_switch_overlay_input(
            &mut state,
            &input_event(VirtualAction::p1_start, InputSource::Keyboard, true),
            &mut effects,
        );
        append_pending_runtime(&mut state, effect, &mut effects);

        assert!(state.profile_switch_overlay.is_none());
    ...
    fn embedded_profile_consumer_forwards_sound_only_effects() {
        let mut overlay = profile_boxes::init(ProfilePickerView::default());
        profile_boxes::set_joined(&mut overlay, false, false);
        let mut effects = Vec::with_capacity(1);
        let outcome = profile_boxes::handle_input(
            &mut overlay,
            &input_event(VirtualAction::p1_start, InputSource::Keyboard, true),
            &mut effects,
        );

        assert_eq!(outcome, profile_boxes::InputOutcome::Continue);
        assert!(matches!(
            effects.as_slice(),
            [ThemeEffect::Runtime(crate::SimplyLoveRuntimeRequest::Audio(
                deadsync_theme::AudioRequest::PlaySfx(path)
    ...
    #[test]
    fn embedded_profile_consumer_suppresses_cancel_navigation() {
        let mut state = init_placeholder();
        let mut overlay = profile_boxes::init(ProfilePickerView::default());
        profile_boxes::set_fast_switch(&mut overlay, true);
        profile_boxes::set_joined(&mut overlay, false, false);
        ...
        let effect = handle_profile_switch_overlay_input(
            &mut state,
            &input_event(VirtualAction::p1_back, InputSource::Keyboard, true),
            &mut effects,
        );
        append_pending_runtime(&mut state, effect, &mut effects);
    ...
    fn late_join_profile_selection_emits_fast_request() {
        ...
        assert_eq!(outcome, profile_boxes::InputOutcome::Selected);
        assert!(matches!(
            &effects[1],
            ThemeEffect::Runtime(crate::SimplyLoveRuntimeRequest::Profile(
                crate::SimplyLoveProfileRequest::Select {
    ...
    fn folder_stats_state() -> super::State {
        ...
        let mut state = init_placeholder();
        state.all_entries = entries.clone().into();
        state.group_entries = entries.clone().into();
    ...
    fn replaygain_prewarm_emits_once_and_precedes_later_media() {
        ...
        let mut state = init_placeholder();
        state.expanded_pack_name = Some(Arc::from("Pack A"));
    ...
    #[test]
    fn confirm_preview_uses_shell_prepared_media_policy() {
        ...
        let ThemeEffect::Batch(effects) = handle_confirm(&mut enabled) else {
            panic!("start sound and prepared preview request should be batched");
        };
        assert!(matches!(
            &effects[0],
            ThemeEffect::Runtime(crate::SimplyLoveRuntimeRequest::Audio(
        ...
        assert!(matches!(
            &effects[1],
            ThemeEffect::Runtime(crate::SimplyLoveRuntimeRequest::Audio(
                deadsync_theme::AudioRequest::PlayMusic { looping: true, .. }
            ))
        ));
        assert_eq!(
    ...
    fn test_entries() -> Vec<super::MusicWheelEntry> {
        vec![
            ...
            super::MusicWheelEntry::PackHeader {
                name: Arc::from("Pack B"),
                original_index: 1,
                banner_path: None,
                song_count: 1,
                pack_key: Some(Arc::from("Pack B")),
                parent_series: None,
            },
    ...
    fn test_pack(name: &str, series: &str) -> SongPack {
        test_pack_with_group(name, name, series)
    }

    fn test_pack_with_group(group_name: &str, name: &str, series: &str) -> SongPack {
        SongPack {
    ...
    fn select_favorites_menu_item(state: &mut super::State) {
        super::show_select_music_menu(state);
        let super::select_music_menu::State::Visible(menu) = &mut state.select_music_menu else {
            panic!("select music menu should be visible");
        };
        menu.selected_index = menu
            ...
            .position(|entry| {
                matches!(
                    entry,
                    super::select_music_menu::Entry::StandaloneItem(item)
                        | super::select_music_menu::Entry::CategoryItem(item)
                        if item.action == super::select_music_menu::Action::SortByFavorites
                )
            })
            .expect("Favorites should be available");
    }

    #[test]
    fn favorites_sort_uses_the_side_that_pressed_start() {
        let mut state = init_placeholder();
        state.session.play_style = profile_data::PlayStyle::Versus;
        state.session.joined = [true, true];
        state.group_entries = test_entries().into();
        state.all_entries = Arc::clone(&state.group_entries);
        state.favorites.pack_names[0].insert("Pack A".to_string());
        ...
        assert_eq!(
            state.sort_mode,
            WheelSortMode::Favorites(profile_data::PlayerSide::P1)
        );
        ...
        assert_eq!(
            state.sort_mode,
            WheelSortMode::Favorites(profile_data::PlayerSide::P2)
        );
        assert_eq!(song_titles(&state.all_entries), ["Song B1"]);
    }
    ...
    fn favorites_view_includes_favorited_pack_without_empty_bucket() {
        ...
        assert!(matches!(
            ...
            super::MusicWheelEntry::PackHeader {
                ...
                ..
            } if name.as_ref() == "Pack B" && key.as_ref() == "Pack B"
        ));
    ...
    fn favorites_view_includes_every_pack_in_favorited_series() {
        ...
        let headers = entries
            .iter()
            .filter_map(|entry| match entry {
                super::MusicWheelEntry::PackHeader { name, .. } => Some(name.as_ref()),
    ...
    fn favorites_view_dedupes_song_favorites_inside_favorited_pack() {
        ...
        assert!(matches!(
            favorites[0],
            super::MusicWheelEntry::PackHeader { ref name, .. } if name.as_ref() == "Pack A"
        ));
    ...
    #[test]
    fn reset_preview_after_gameplay_preserves_selection_and_rearms_refresh() {
        let mut state = init_placeholder();
        state.entries = test_entries();
    ...
    #[test]
    fn select_menu_left_right_changes_difficulty_instead_of_moving_wheel() {
        let mut state = init_placeholder();
        ...
        let now = Instant::now();
        let mut effects: Vec<ThemeEffect> = Vec::new();
        super::handle_input(
            &mut state,
            &input_event_at(
                VirtualAction::p1_select,
        ...
        // Releasing a swallowed press must not step the wheel or start a hold.
        super::handle_input(
            &mut state,
            &input_event_at(
                VirtualAction::p1_menu_left,
        ...
        // Pad Left/Right are not remapped: with Select still held they move
        // the wheel as usual and leave the chart alone.
        super::handle_input(
            &mut state,
            &input_event_at(
                VirtualAction::p1_left,
        ...
        // With Select released, MenuLeft moves the wheel again.
        super::handle_input(
            &mut state,
            &input_event_at(
                VirtualAction::p1_select,
    ...
    #[test]
    fn versus_select_menu_left_right_steps_each_side_independently() {
        let mut state = init_placeholder();
        state.session.play_style = profile_data::PlayStyle::Versus;
        state.session.joined = [true, true];
        state.entries = charted_song_entries();
        ...
        // P1 has no Select held, so their Right still moves the wheel.
        super::handle_input(
            &mut state,
            &input_event_at(
                VirtualAction::p1_right,
    ...
    fn up_down_chord_closes_expanded_pack_from_header() {
        let mut state = init_placeholder();
        state.all_entries = test_entries().into();
        ...
        assert!(matches!(
            state.entries.get(state.selected_index),
            Some(super::MusicWheelEntry::PackHeader { name, .. }) if name.as_ref() == "Pack A"
        ));
    ...
    fn itg_wheel_style_keeps_other_pack_headers_visible() {
        ...
        assert!(matches!(
            entries[3],
            super::MusicWheelEntry::PackHeader { ref name, .. } if name.as_ref() == "Pack B"
        ));
    }

    #[test]
    fn fast_internal_lookups_preserve_select_music_keys() {
        let entries = test_entries();
        let cache = super::FolderStatsCache::new(&entries);
        assert_eq!(cache.pack_by_key.get("Pack A"), Some(&0));
        assert_eq!(cache.pack_by_key.get("Pack B"), Some(&1));
    ...
    #[test]
    fn sort_switch_shares_cached_entries_and_header_storage() {
        let mut state = init_placeholder();
        state.sort_mode = WheelSortMode::Title;
        state.group_entries = test_entries().into();
        ...
        assert!(Arc::ptr_eq(&state.all_entries, &state.group_entries));
        assert_eq!(state.expanded_pack_name.as_deref(), Some("Pack A"));
        assert_eq!(song_titles(&state.entries), ["Song A1", "Song A2"]);
        let super::MusicWheelEntry::PackHeader {
            name: cached_name,
            pack_key: Some(cached_key),
            ..
        } = &state.group_entries[0]
        else {
            panic!("group cache should begin with a keyed pack header");
        };
        let super::MusicWheelEntry::PackHeader {
            name: visible_name,
            pack_key: Some(visible_key),
            ..
        } = &state.entries[0]
        else {
            panic!("visible entries should preserve the pack header");
        };
        let expanded = state
            .expanded_pack_name
            .as_ref()
            .expect("sort should open its first pack");
        assert!(Arc::ptr_eq(cached_key, expanded));
        assert!(Arc::ptr_eq(cached_name, visible_name));
        assert!(Arc::ptr_eq(cached_key, visible_key));
    }
    ...
    fn visible_wheel_refill_reuses_capacity() {
        ...
        *banner_path = Some(Arc::clone(&shared_banner));
        let mut visible = Vec::with_capacity(all_entries.len());
        let storage = visible.as_ptr();
    ...
    fn series_sort_nests_packs_from_pack_metadata() {
        ...
        assert!(matches!(
            ...
            super::MusicWheelEntry::PackHeader {
                ...
                ..
            } if name.as_ref() == "ITG Series" && parent.as_ref() == "ITG Series"
        ));
        assert!(matches!(
            entries[1],
            super::MusicWheelEntry::PackHeader {
                ...
                parent_series: Some(ref parent),
                ..
            } if name.as_ref() == "Pack A" && parent.as_ref() == "ITG Series"
        ));
        assert!(matches!(
            entries[4],
            super::MusicWheelEntry::PackHeader {
    ...
    fn series_parent_preserves_child_pack_palette_indices() {
        ...
        assert!(matches!(
            entries.iter().find(|entry| {
                matches!(entry, super::MusicWheelEntry::PackHeader { name, .. } if name.as_ref() == "Pack A")
            }),
            Some(super::MusicWheelEntry::PackHeader {
                original_index: 0,
                ..
        ...
        assert!(matches!(
            entries.iter().find(|entry| {
                matches!(entry, super::MusicWheelEntry::PackHeader { name, .. } if name.as_ref() == "Pack B")
            }),
            Some(super::MusicWheelEntry::PackHeader {
                original_index: 1,
                ..
    ...
    #[test]
    fn hide_inactive_series_focuses_open_series() {
        let packs = [
            test_pack("Pack A", "ITG Series"),
            test_pack("Pack B", "ITG Series"),
        ];
        ...
        for wheel_style in [SelectMusicWheelStyle::Itg, SelectMusicWheelStyle::Iidx] {
            let shown =
                build_displayed_entries(&entries, Some("ITG Series"), None, wheel_style, true);

            assert_eq!(shown.len(), 3);
            assert!(shown[0].is_series_header());
            assert!(shown.iter().all(|entry| {
                !matches!(
                    entry,
                    super::MusicWheelEntry::PackHeader { name, .. } if name.as_ref() == "Loose Pack"
                )
            }));
        }
    }
    ...
    fn series_back_navigation_closes_one_level() {
        ...
        let mut state = init_placeholder();
        state.all_entries = super::build_series_grouped_entries(&test_entries(), &packs).into();
        state.expanded_series_name = Some(Arc::from("ITG Series"));
        state.expanded_pack_name = Some(Arc::from("Pack A"));
        super::rebuild_displayed_entries(&mut state);
    ...
    fn iidx_wheel_style_only_shows_active_pack_and_header() {
        ...
        assert!(entries.iter().all(|entry| {
            !matches!(
                entry,
                super::MusicWheelEntry::PackHeader { name, .. } if name.as_ref() == "Pack B"
            )
        }));
    }
    ...
    fn duplicate_pack_display_titles_open_by_group_key() {
        ...
        let mut lower = test_pack_with_group("Stamina RPG 9 - Unlocks", "Stamina RPG 9", "SRPG");
        lower.songs.push(lower_song.clone());
        let mut init_view = super::SelectMusicInitView {
        ...
        assert_eq!(
            state.expanded_pack_name.as_deref(),
            Some("Stamina RPG 9 - Unlocks")
        );
        assert_eq!(
            state.entries[state.selected_index].pack_key(),
            Some("Stamina RPG 9 - Unlocks")
        );
        assert!(state.entries.iter().any(
            |entry| matches!(entry, super::MusicWheelEntry::Song(song) if Arc::ptr_eq(song, &lower_song))
        ));
        assert!(!state.entries.iter().any(
            |entry| matches!(entry, super::MusicWheelEntry::Song(song) if Arc::ptr_eq(song, &upper_song))
        ));
    }

    #[test]
    fn iidx_pack_focus_hides_series_above_loose_pack() {
        let packs = [
            test_pack("Pack A", "ITG Series"),
            test_pack("Pack B", "ITG Series"),
        ];
        ...
        for hide_inactive_series in [false, true] {
            ...
            assert!(matches!(
                ...
                super::MusicWheelEntry::PackHeader { ref name, .. }
                    if name.as_ref() == "ITGAlex's Compilation"
            ));
    ...
    fn itg_loose_pack_open_clears_parent_series() {
        ...
        let mut all_entries = super::build_series_grouped_entries(&test_entries(), &packs);
        all_entries.push(header("Loose Pack", 2, 1, Some("Loose Pack")));
        all_entries.push(super::MusicWheelEntry::Song(test_song("Loose Song")));
        state.all_entries = all_entries.into();
        state.expanded_series_name = Some(Arc::from("ITG Series"));
        super::rebuild_displayed_entries(&mut state);
        state.selected_index = state
            .entries
    ...
    #[test]
    fn iidx_pack_focus_keeps_only_active_parent_when_option_off() {
        let grouped_entries = vec![
            header("Pack A", 0, 1, Some("Pack A")),
            super::MusicWheelEntry::Song(test_song("Song A")),
            header("Pack B", 1, 1, Some("Pack B")),
        ...
        assert_eq!(headers, ["Series A", "Pack A"]);
        assert_eq!(song_titles(&shown), ["Song A"]);
    }
    ...
    fn playlist_parser_supports_sections_and_pack_wildcards() {
        ...
        assert!(matches!(
            playlist_entries[0],
            super::MusicWheelEntry::PackHeader { ref name, song_count: 2, .. } if name.as_ref() == "Warmup"
        ));
        assert!(matches!(
            playlist_entries[1],
        ...
        assert!(matches!(
            playlist_entries[3],
            super::MusicWheelEntry::PackHeader { ref name, song_count: 1, .. } if name.as_ref() == "Finale"
        ));
        assert!(matches!(
            playlist_entries[4],
    ...
    fn playlist_parser_uses_playlist_name_when_no_header_exists() {
        ...
        assert!(matches!(
            playlist_entries[0],
            super::MusicWheelEntry::PackHeader { ref name, song_count: 2, .. } if name.as_ref() == "Night Shift"
        ));
        assert!(matches!(
            playlist_entries[1],
            super::MusicWheelEntry::Song(ref song) if song.title == "Beta"
