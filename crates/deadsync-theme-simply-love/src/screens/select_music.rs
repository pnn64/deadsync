...
mod sync_graph;
use sync_graph::{
    SYNC_HEAT_ALPHA, SyncGraphCols, build_sync_heat_image, sync_graph_cols, update_sync_curve_mesh,
};
...
struct NullOrDieOverlayData {
    ...
    post_kernel: Vec<f64>,
    convolution: Vec<f64>,
    curve_mesh: Option<Arc<Vec<MeshVertex>>>,
    edge_discard: usize,
...
fn refresh_sync_overlay_curve_mesh(overlay: &mut NullOrDieOverlayData) {
    let (graph_w, graph_h) = sync_overlay_graph_size();
    let cols = sync_overlay_graph_cols(overlay);
    update_sync_curve_mesh(
        &mut overlay.curve_mesh,
        &overlay.convolution,
...
fn handle_raw_key_event_impl(
    ...
    if state.pad_config_overlay_visible {
        ...
        // `0` tares the focused threshold(s) to the live reading, like Up+Down.
        if let Some(k) = key
