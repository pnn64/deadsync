...
const GAME_TIME_WIDTH_BASELINE_SLOT: usize = 4;
const PADDED_NUM_SLOT_COUNT: usize = FRAME_TEXT_STATS_COUNT_ROWS as usize * 3;
...
// never allocate, scan-prune, or destroy heap resources. Stable HUD actors use
// their prepared text slot directly: disabled full-width counts use 21 compact
// actor slots, clocks use four row slots, live timing uses six stat
// slots, and clock widths use four clock-row slots plus one baseline slot. A
// miss performs one key comparison plus <=14 bytes of stack formatting, with
...
thread_local! {
    ...
    static PADDED_NUM_SLOT_CACHE: RefCell<SlotTextCache<(u32, u8), InlineText, PADDED_NUM_SLOT_COUNT>> = const {
        RefCell::new(SlotTextCache::new())
    };
    static GAME_TIME_SLOT_CACHE: RefCell<SlotTextCache<(u32, u8), InlineText, 4>> = const {
        RefCell::new(SlotTextCache::new())
    };
    static GAME_TIME_WIDTH_SLOT_CACHE: RefCell<SlotTextCache<(u32, u8), f32, 5>> = const {
        RefCell::new(SlotTextCache::new())
    };
...
const STANDARD_JUDGMENT_ROLES: [JudgmentColorRole; 6] = [
    JudgmentColorRole::FantasticBlue,
    JudgmentColorRole::Excellent,
    JudgmentColorRole::Great,
    JudgmentColorRole::Decent,
    JudgmentColorRole::WayOff,
    JudgmentColorRole::Miss,
];

#[inline(always)]
fn standard_judgment_color(palette: JudgmentPalette, index: usize) -> [f32; 4] {
    palette.color(STANDARD_JUDGMENT_ROLES[index.min(STANDARD_JUDGMENT_ROLES.len() - 1)])
}

#[inline(always)]
fn standard_judgment_dim_color(palette: JudgmentPalette, index: usize) -> [f32; 4] {
    palette
        .gameplay_dim_color(STANDARD_JUDGMENT_ROLES[index.min(STANDARD_JUDGMENT_ROLES.len() - 1)])
}
...
impl GameplayStatsTextPlan {
    ...
    fn from_parts_with_derived(
        ...
        Self {
            ...
            judgments: std::array::from_fn(|index| JUDGMENT_LABELS[index].get()),
...
mod gameplay_stats_text_plan_tests {
    ...
    fn prewarmed_text_preserves_labels_and_description_cycle() {
        ...
        assert_eq!(plan.judgment(0).as_ref(), JUDGMENT_LABELS[0].get().as_ref());
        assert_eq!(
...
#[inline(always)]
fn format_padded_num_inline(count: u32, digits: usize) -> InlineText {
...
fn padded_num_text(count: u32, digits: usize) -> TextContent {
    ...
    if digits <= InlineText::CAPACITY {
        ...
        let text = PADDED_NUM_INLINE_CACHE.with(|cache| {
            cache
                .borrow_mut()
                .get_or_insert(key, || format_padded_num_inline(count, digits))
        });
...
#[inline(always)]
fn padded_num_text_in_slot(count: u32, digits: usize, slot: u8) -> TextContent {
    let digits = digits.clamp(1, u8::MAX as usize);
    if digits <= InlineText::CAPACITY {
        let key = (count, digits as u8);
        let text = PADDED_NUM_SLOT_CACHE.with(|cache| {
            let cache_slot = usize::from(slot.saturating_sub(FRAME_TEXT_STATS_COUNT_LEFT_BASE))
                .min(PADDED_NUM_SLOT_COUNT - 1);
            cache
                .borrow_mut()
                .get_or_insert(cache_slot, key, || format_padded_num_inline(count, digits))
        });
        TextContent::frame_inline_slot(text, slot)
    } else {
        TextContent::Owned(format!("{count:0digits$}"))
    }
}
...
fn padded_runs_for_window(
    ...
    if disabled {
        (
            padded_num_text_in_slot(count, digits, slot),
            TextContent::default(),
        )
    } else {
...
mod dynamic_hud_text_tests {
    ...
    fn padded_counts_preserve_zero_padding_and_disabled_window_behavior() {
        ...
        for count in [0, 1, 9, 10, 99, 100, 9_999, 10_000, u32::MAX] {
            for digits in 1..=InlineText::CAPACITY {
                let slot = count_text_slot(count as usize % 2, digits % 7, false);
                let text = super::padded_num_text_in_slot(count, digits, slot);
                assert_eq!(text.as_str(), format!("{count:0digits$}"));
                assert!(matches!(
                    text,
                    TextContent::FrameInline { slot: found, .. } if found == slot
                ));
            }
        }
...
#[inline(always)]
fn cached_game_time_width_for_key(
    key: (u32, u8),
    slot: usize,
    asset_manager: &AssetManager,
) -> f32 {
    GAME_TIME_WIDTH_SLOT_CACHE.with(|cache| {
        cache.borrow_mut().get_or_insert(slot, key, || {
...
pub fn prewarm_text_layout(
    ...
    let _ = cached_game_time_width_for_key((end_seconds, mode), 0, asset_manager);
    let _ = cached_game_time_width_for_key((end_seconds, mode), 2, asset_manager);
    let _ = cached_game_time_width_for_key(
        (9 * 60 + 59, 2),
        GAME_TIME_WIDTH_BASELINE_SLOT,
        asset_manager,
    );
    for player in 0..state.num_players() {
        ...
        let _ = cached_game_time_width_for_key(key, player * 2 + 1, asset_manager);
    }
...
fn prewarm_count_slots(
    ...
    for player in 0..num_players.min(MAX_PLAYERS) {
        for row in 0..FRAME_TEXT_STATS_COUNT_ROWS {
            ...
            let _ = padded_num_text_in_slot(0, 4, slot);
            prewarm_prepared_inline_text_slot(
                cache,
                scratch,
                fonts,
    ...
    if num_players > 1 {
        for row in 0..FRAME_TEXT_STATS_COUNT_ROWS {
            let slot = FRAME_TEXT_STATS_COUNT_RIGHT_BASE + row;
            let _ = padded_num_text_in_slot(0, 4, slot);
            prewarm_prepared_inline_text_slot(
                cache,
                scratch,
...
pub fn push_versus_step_stats(
            ...
            asset_manager.with_font(gameplay_font_key(state, FontRole::ScreenEval), |f| {
                ...
                for (player_idx, show) in show_judgments_for.iter().copied().enumerate() {
                    ...
                    if show_fa_split && end > start {
                        ...
                        let palette = state.judgment_palette(player_idx);
                        let bright_colors = palette.colors;
                        ...
                        for (row_i, count) in counts.iter().copied().enumerate() {
                            ...
                            let dim_color = if disabled {
                                ...
                                standard_judgment_dim_color(
                                    state.judgment_palette(player_idx),
                                    row_i,
                                )
                            };
                            let bright_color = if disabled {
                                ...
                                standard_judgment_color(state.judgment_palette(player_idx), row_i)
                            };
                            push_versus_count_texts(
                                actors,
                                state,
    ...
    for (player_idx, show) in show_judgments_for.iter().copied().enumerate() {
        ...
        let score_mode = score_display_mode_from_profile(player_profile.score_display_mode);
        // EX and H.EX share one score snapshot.
        let ex_score = player_profile.show_ex_score.then(|| {
            state.display_scored_ex_score_data(
                player_idx,
                state.gameplay_stats_text.blue_window_ms(player_idx),
            )
        });
        let (score_value, mut score_color) = if let Some(ex_score) = &ex_score {
            (
                deadsync_gameplay::display_ex_score_percent_for_mode(ex_score, score_mode).max(0.0),
                state
                    .judgment_palette(player_idx)
                    .color(JudgmentColorRole::FantasticBlue),
            )
        } else {
            let score_percent = state.display_gameplay_itg_score_percent(player_idx, score_mode);
        ...
        if player_profile.show_hard_ex_score
            && let Some(ex_score) = &ex_score
        {
            let hard_ex_percent =
                deadsync_gameplay::display_hard_ex_score_percent_for_mode(ex_score, score_mode);
...
pub fn push_double_step_stats(
                ...
                if show_standard_judgments {
                    ...
                    for (row_i, count) in counts.into_iter().enumerate() {
                        ...
                        let bright = if disabled {
                            ...
                            standard_judgment_color(state.judgment_palette(0), row_i)
                        };
                        let dim = if disabled {
                            DISABLED_WINDOW_RGBA
                        } else {
                            standard_judgment_dim_color(state.judgment_palette(0), row_i)
                        };
                    ...
                    let counts = [wc.w0, wc.w1, wc.w2, wc.w3, wc.w4, wc.w5, wc.miss];
                    let palette = state.judgment_palette(0);
                    let bright_colors = palette.colors;
                    let dim_colors = palette.gameplay_dim;
    ...
    if mask.contains(profile_data::StepStatisticsMask::SONG_DURATION) {
        ...
        let total_w = cached_game_time_width_for_key(total_time_key, 0, asset_manager);
...
fn build_side_pane(
    ...
    if show_judgments || show_duration {
        ...
        if show_judgments && show_standard_judgments {
            ...
            for (index, grade) in JUDGMENT_ORDER.iter().enumerate() {
                ...
                let dim = if disabled {
                    ...
                    standard_judgment_dim_color(state.judgment_palette(player_idx), index)
                };
            ...
            let palette = state.judgment_palette(player_idx);
            let dim_fantastic = palette.gameplay_dim_color(JudgmentColorRole::FantasticBlue);
            let dim_excellent = palette.gameplay_dim_color(JudgmentColorRole::Excellent);
            let dim_great = palette.gameplay_dim_color(JudgmentColorRole::Great);
            let dim_decent = palette.gameplay_dim_color(JudgmentColorRole::Decent);
            let dim_wayoff = palette.gameplay_dim_color(JudgmentColorRole::WayOff);
            let dim_miss = palette.gameplay_dim_color(JudgmentColorRole::Miss);
            let dim_white_fa = palette.gameplay_dim_color(JudgmentColorRole::FantasticWhite);

            let white_fa_color = palette.color(JudgmentColorRole::FantasticWhite);
        ...
        if show_duration {
            ...
            let total_width_px = cached_game_time_width_for_key(
                total_time_key,
                time_slot,
                asset_manager,
            ) * time_value_zoom;
            let remaining_width_px = cached_game_time_width_for_key(
                remaining_time_key,
                time_slot + 1,
                asset_manager,
            ) * time_value_zoom;
            // Use "9:59" as the baseline look the layout was tuned for.
            let baseline_width_px = cached_game_time_width_for_key(
                (9 * 60 + 59, 2),
                GAME_TIME_WIDTH_BASELINE_SLOT,
                asset_manager,
            ) * time_value_zoom;
