...
// bytes on 32-bit, 8 bytes on 64-bit) rather than being fixed at 64 bits.
// Mismatching this silently misaligns every field after the first event in a
// read buffer.
...
fn add_dev_if_new(
    ...
    let id =
        existing_id.unwrap_or_else(|| host.pad_id_for_uuid(PadOrderBackend::FreeBsdEvdev, uuid));
        ...
        match spec.class {
            DevClass::Pad => {
                ...
                let id = match id_by_uuid.get(&uuid).copied() {
                    Some(id) => id,
                    None => host.pad_id_for_uuid(PadOrderBackend::FreeBsdEvdev, uuid),
                };
