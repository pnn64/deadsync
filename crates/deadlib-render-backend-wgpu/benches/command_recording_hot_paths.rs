use deadlib_render_backend_wgpu::bench_support;
...
fn main() {
    let vertex_events = bench_support::vertex_events(EVENTS);
    print_pair(
        "WGPU draw vertex accounting",
        vertex_events.len(),
        &measure(2_048, || {
            bench_support::vertices_old(black_box(&vertex_events))
        }),
        &measure(2_048, || {
            bench_support::vertices_new(black_box(&vertex_events))
        }),
    );

    let projections_a = bench_support::projection_fixture(16, 8, 0.0);
    let projections_b = bench_support::projection_fixture(16, 8, 0.25);
    let projection_bytes = bench_support::projection_bytes(&projections_a);
    let mut old_upload = Vec::with_capacity(projection_bytes);
    let mut new_upload = Vec::with_capacity(projection_bytes);
    let mut old_phase = false;
    let mut new_phase = false;
    print_pair(
        "WGPU changing offscreen projection staging",
        16 * 9,
        &measure(2_048, || {
            old_phase = !old_phase;
            let fixture = if old_phase {
                &projections_a
            } else {
                &projections_b
            };
            bench_support::projections_old(black_box(fixture), &mut old_upload)
        }),
        &measure(2_048, || {
            new_phase = !new_phase;
            let fixture = if new_phase {
                &projections_a
            } else {
                &projections_b
            };
            bench_support::projections_new(black_box(fixture), &mut new_upload)
        }),
    );
    ...
    let mut new_upload = Vec::with_capacity(projection_bytes);
    print_pair(
        "WGPU unchanged offscreen projection staging",
        16 * 9,
        &measure(2_048, || {
            bench_support::projections_old(black_box(&projections_a), &mut old_upload)
        }),
        &measure(2_048, || {
            bench_support::projections_new(black_box(&projections_a), &mut new_upload)
        }),
    );
    println!("  queue writes: 1 old -> 0 new after the first unchanged frame");
}
