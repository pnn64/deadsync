...
pub use present_dsl::{__dsl_parse_effect_clock, TextBuilder};
...
impl SpriteBuilder {
    ...
    fn with_source(source: SpriteSource) -> Self {
        Self {
            inner: present_dsl::SpriteBuilder::with_source(source),
        }
    }
    ...
    pub fn zoomto(&mut self, w: f32, h: f32) {
        self.inner.zoomto_with_native_dims(w, h, self.native_dims());
    }

    #[inline(always)]
    pub fn build(self, site_base: u64) -> Actor {
        let native_dims = self.native_dims();
        self.inner
            .build_with_native_dims(site_base, Some(native_dims))
    }
}
