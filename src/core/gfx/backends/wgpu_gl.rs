...
pub fn init(window: Arc<Window>, vsync_enabled: bool) -> Result<State, Box<dyn Error>> {
    info!("Initializing OpenGL (wgpu) backend...");
...
pub fn draw(
        ...
        match &obj.object_type {
            ObjectType::Sprite {
                ...
                let tex = match textures.get(texture_id.as_ref()) {
                    Some(RendererTexture::OpenGLWgpu(t)) => t,
                    _ => continue,
                };
    ...
    Ok((instances.len() as u32) * 4 + (mesh_vertices.len() as u32))
}
...
pub fn cleanup(_state: &mut State) {
    info!("OpenGL (wgpu) backend cleanup complete.");
}
...
fn pick_format(caps: &wgpu::SurfaceCapabilities) -> wgpu::TextureFormat {
    // Match other GPU paths: avoid sRGB conversion to keep colors consistent.
    caps.formats
        .iter()
        .copied()
        .find(|f| !f.is_srgb())
...
fn build_pipeline_set(
    ...
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("gl shader module"),
        source: wgpu::ShaderSource::Wgsl(Cow::Borrowed(SHADER)),
    });

    let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("gl pipeline layout"),
...
fn build_mesh_pipeline_set(
    ...
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("gl mesh shader module"),
        source: wgpu::ShaderSource::Wgsl(Cow::Borrowed(MESH_SHADER)),
    });

    let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("gl mesh pipeline layout"),
...
const SHADER: &str = include_str!("../shaders/wgpu_sprite_ubo.wgsl");
const MESH_SHADER: &str = include_str!("../shaders/wgpu_mesh_ubo.wgsl");
