...
unsafe extern "C" {
    ...
    fn jack_client_close(client: *mut JackClientRaw) -> c_int;
    fn jack_activate(client: *mut JackClientRaw) -> c_int;
    fn jack_deactivate(client: *mut JackClientRaw) -> c_int;
    fn jack_set_process_callback(
        client: *mut JackClientRaw,
        process_callback: unsafe extern "C" fn(JackNFrames, *mut c_void) -> c_int,
        arg: *mut c_void,
    ) -> c_int;
    fn jack_get_sample_rate(client: *mut JackClientRaw) -> JackNFrames;
    fn jack_get_buffer_size(client: *mut JackClientRaw) -> JackNFrames;
    fn jack_get_ports(
        client: *mut JackClientRaw,
    ...
    fn jack_port_unregister(client: *mut JackClientRaw, port: *mut JackPortRaw) -> c_int;
    fn jack_port_name(port: *const JackPortRaw) -> *const c_char;
    fn jack_port_get_buffer(port: *mut JackPortRaw, nframes: JackNFrames) -> *mut c_void;
...
impl JackClient {
    ...
    fn deactivate(&mut self) {
        if self.activated && !self.raw.is_null() {
            unsafe { jack_deactivate(self.raw) };
        ...
        if !self.raw.is_null() {
            unsafe {
                if !self.port_r.is_null() {
                    jack_port_unregister(self.raw, self.port_r);
                }
                if !self.port_l.is_null() {
                    jack_port_unregister(self.raw, self.port_l);
                }
                jack_client_close(self.raw);
            }
        }
    }
}
...
impl JackCallbackState {
    ...
    fn process(&mut self, nframes: u32) {
        ...
        let left = unsafe { port_buffer(self.port_l, nframes) };
        let right = unsafe { port_buffer(self.port_r, nframes) };
        for i in 0..frames {
...
pub(crate) fn prepare(
    ...
    let sample_rate_hz = unsafe { jack_get_sample_rate(client) as u32 }.max(1);
    let period_frames = unsafe { jack_get_buffer_size(client) as u32 }.max(1);
...
pub(crate) fn start(
    ...
    if unsafe { jack_activate(client.raw) } != 0 {
        unsafe { drop(Box::from_raw(callback_state)) };
...
unsafe extern "C" fn jack_process_callback(nframes: JackNFrames, arg: *mut c_void) -> c_int {
    ...
    let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| state.process(nframes)))
        .map_err(|_| unsafe {
            zero_port(state.port_l, nframes);
            zero_port(state.port_r, nframes);
        });
    0
}
...
fn connect_physical_playback(client: &JackClient) -> Result<(), String> {
    ...
    if port_out_l.is_null() {
        unsafe { jack_free(ports.cast::<c_void>()) };
    ...
    let left_connect =
        unsafe { jack_connect(client.raw, jack_port_name(client.port_l), port_out_l) };
    let right_connect =
        unsafe { jack_connect(client.raw, jack_port_name(client.port_r), port_out_r) };
    unsafe { jack_free(ports.cast::<c_void>()) };
...
#[inline(always)]
unsafe fn port_buffer(port: *mut JackPortRaw, nframes: JackNFrames) -> &'static mut [f32] {
    unsafe {
        slice::from_raw_parts_mut(
            jack_port_get_buffer(port, nframes).cast::<f32>(),
            nframes as usize,
        )
    }
}
...
#[inline(always)]
unsafe fn zero_port(port: *mut JackPortRaw, nframes: JackNFrames) {
    unsafe { port_buffer(port, nframes).fill(0.0) };
}
