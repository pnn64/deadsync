...
mod tests {
    ...
    fn for_grade_picks_the_right_animation_and_falls_back() {
        ...
        // A grade without a gif yields None: that event shows no panel effect.
    ...
    fn sustain_anim_distinguishes_freeze_and_roll() {
        ...
        // No gifs at all: no sustain effect is played for either kind.
    ...
    fn results_role_candidates_follow_the_documented_chain() {
        ...
        // A +/- grade with a difficulty: difficulty-qualified first at each
        // tier, then the base letter, then difficulty alone.
        assert_eq!(
        ...
        // No difficulty: grade tiers only.
        assert_eq!(
            results_role_candidates(Grade::Tier05, None),
            ["results@S+", "results@S"]
        );
        // A base grade has no base-letter fallback of its own.
        assert_eq!(
