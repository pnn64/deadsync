...
mod tests {
    ...
    #[test]
    fn compute_column_judgments_uses_row_judgment_for_jump_columns() {
        let notes = [
            tap_note(
                0,
    ...
    #[test]
    fn compute_column_judgments_keeps_miss_priority_for_jump_columns() {
        let notes = [
    ...
    #[test]
    fn compute_column_judgments_keeps_held_miss_on_raw_column() {
...
pub fn init(gameplay_results: Option<gameplay::State>) -> State {
    ...
    if let Some(mut gs) = gameplay_results {
            ...
            .take(gs.num_players.min(MAX_PLAYERS))
        {
            ...
            let foot_parity =
                crate::game::gameplay::foot_parity_by_row_for_results(&gs, player_idx);
            if !foot_parity.is_empty() {
                for sp in &mut scatter {
                    if let Some(&placement) = foot_parity.get(&sp.row_index) {
                        sp.parity_foot = placement;
                    }
                }
            }
