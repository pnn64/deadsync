...
//! regions are in SM-logical (top-left origin) coordinates. The pointer
...
#[inline]
pub fn hit_test(rects: &[HitRect], point: (f32, f32)) -> Option<u32> {
...
mod tests {
    ...
    fn contains_includes_borders() {
        ...
        assert!(!r.contains((20.0, 19.9)));
        assert!(!r.contains((20.0, 40.1)));
    }

    #[test]
    fn from_center_round_trips() {
        let r = HitRect::from_center(100.0, 200.0, 50.0, 20.0, 7);
        assert_eq!(r.min, (75.0, 190.0));
        assert_eq!(r.max, (125.0, 210.0));
        assert!(r.contains((100.0, 200.0)));
        assert!(!r.contains((74.9, 200.0)));
    }
    ...
    fn hit_test_picks_topmost_overlap() {
        ...
        assert_eq!(hit_test(&rects, (32.0, 32.0)), Some(1));
        // Inside both — topmost (id=2) wins.
        assert_eq!(hit_test(&rects, (50.0, 50.0)), Some(2));
        // Outside both.
        assert_eq!(hit_test(&rects, (100.0, 100.0)), None);
    }
