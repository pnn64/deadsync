    ...
    asset_manager.with_fonts(|all_fonts| asset_manager.with_font(current_theme_font_key(FontRole::ScreenEval), |metrics_font| {
        ...
        if show_standard_judgments {
            for (i, info) in JUDGMENT_INFO.iter().enumerate() {
                ...
                for (char_idx, digit) in digits.iter().take(digits_to_fmt).enumerate() {
                    ...
                    actors.push(act!(text: font(current_theme_font_key(FontRole::ScreenEval)): settext(digit_text(*digit)):
        ...
        for (i, (_, achieved, possible)) in radar_categories.iter().copied().enumerate() {
            ...
            for char_idx_from_right in 0..3 {
                ...
                actors.push(act!(text: font(current_theme_font_key(FontRole::ScreenEval)): settext(digit_text(digits[digit_idx])):
            ...
            actors.push(act!(text: font(current_theme_font_key(FontRole::ScreenEval)): settext(SLASH_TEXT.clone()):
