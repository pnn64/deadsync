...
pub fn get_actors(state: &State, asset_manager: &AssetManager) -> Vec<Actor> {
    ...
    for slot in 0..VISIBLE_STEPS_SLOTS {
        ...
        if let Some(chart) = steps_charts[idx] {
            ...
            actors.push(act!(text: font(current_theme_font_key(FontRole::Header)): settext(chart.meter.to_string()): align(0.5, 0.5): xy(lst_cx, lst_cy + y): zoom(0.45): z(122): diffuse(c[0], c[1], c[2], 1.0)));
        }
    }
