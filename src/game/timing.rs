...
pub fn parse_fakes(s: &str) -> Result<Vec<FakeSegment>, &'static str> {
    ...
    for part in s.split(',') {
        ...
        let Some(beat) = parse_f32_fast(beat_str) else {
            continue;
        };
        let Some(len) = parse_f32_fast(len_str) else {
            continue;
        };
...
fn process_bpms_and_stops(
    ...
    let mut stop_changes: Vec<(f32, f32)> = stops.iter().map(|s| (s.beat, s.duration)).collect();
