...
const CO_TASK_MEM_FREE: &[u8; 14] = b"CoTaskMemFree\0";
...
unsafe fn free_com_task_mem(ptr: windows::core::PWSTR) {
    if ptr.is_null() {
        return;
    }

    // `windows-sys` 0.61 imports CoTaskMemFree from combase.dll, which is not
    // present on Windows 7. Resolve the same API from ole32.dll at runtime.
    let ole32 = unsafe { LoadLibraryW(OLE32_DLL.as_ptr()) };
    if ole32.is_null() {
        return;
    }

    let proc = unsafe { GetProcAddress(ole32, CO_TASK_MEM_FREE.as_ptr()) };
    if !proc.is_null() {
