...
use windows_sys::Win32::Foundation::{
    CloseHandle, DEVPROPKEY, FALSE, HANDLE, INVALID_HANDLE_VALUE, PROPERTYKEY, TRUE, WAIT_OBJECT_0,
    WAIT_TIMEOUT,
};
...
impl Overlapped {
    ...
    pub fn get_result(&mut self, handle: &Handle, timeout: Option<u32>) -> WinResult<usize> {
        // `GetOverlappedResultEx` is Windows 8+, so wait separately and then
        // collect the completed overlapped result for Windows 7 compatibility.
        let wait = unsafe { WaitForSingleObject(self.event_handle(), timeout.unwrap_or(INFINITE)) };
        if wait == WAIT_TIMEOUT {
            return Err(WinError::WaitTimedOut);
        }
        ensure!(wait == WAIT_OBJECT_0, Err(WinError::last()));

        let mut bytes_written = 0;
        let cr = unsafe {
            GetOverlappedResult(handle.as_raw(), self.as_raw(), &mut bytes_written, FALSE)
        };
